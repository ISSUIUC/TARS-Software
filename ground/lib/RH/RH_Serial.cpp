...
void RH_Serial::waitAvailable() {
    ...
    // Unix version driver in RHutil/HardwareSerial knows how to wait without
    // polling
...
void RH_Serial::appendRxBuf(uint8_t ch) {
    ...
    // If the buffer overflows, we dont record the trailing data, and the FCS
    // will be wrong, causing the message to be dropped when the FCS is received
}
