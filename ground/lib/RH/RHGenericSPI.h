...
///
/// This generic abstract class is used to encapsulate hardware or software SPI
/// interfaces for a variety of platforms. The intention is so that driver
/// classes can be configured to use hardware or software SPI without changing
/// the main code.
///
/// You must provide a subclass of this class to driver constructors that
/// require SPI. A concrete subclass that encapsualates the standard Arduino
/// hardware SPI and a bit-banged software implementation is included.
///
/// Do not directly use this class: it must be subclassed and the following
/// abstract functions at least must be implmented:
/// - begin()
/// - end()
/// - transfer()
class RHGenericSPI {
   public:
    ...
    /// We need to define these in a device and platform independent way,
    /// because the SPI implementation is different on each platform.
    typedef enum {
    ...
    /// Do not use this contructor directly: you must instead use on of the
    /// concrete subclasses provided such as RHHardwareSPI or RHSoftwareSPI
    ...
    /// frequency on the platform. \param[in] bitOrder Select the SPI bus bit
    /// order, one of RHGenericSPI::BitOrderMSBFirst or
    ...
    /// This can be used in an SPI slave to indicate when an SPI message has
    /// been received
    virtual void attachInterrupt(){};

    /// Disable SPI interrupts (if supported)
    /// This can be used to diable the SPI interrupt in slaves where that is
    /// supported.
    ...
    /// first). \param[in] bitOrder Bit order to be used: one of
    ...
    /// On AVR based boards, the dividers available are 2, 4, 8, 16, 32, 64 or
    /// 128. The default setting is SPI_CLOCK_DIV4, which sets the SPI clock to
    /// one-quarter the frequency of the system clock (4 Mhz for the boards at
    /// 16 MHz). \param[in] frequency The data rate to use: one of
