...
// transceivers (see http://www.hoperf.com) RHDatagram will be received only by
// the addressed node or all nodes within range if the to address is
        ...
        if (waitAvailableTimeout(timeLeft)) {
            if (RHRouter::recvfromAck(_tmpMessage, &messageLen)) {
                if (messageLen > 1 &&
                    ...
                    // Got a reply, now add the next hop to the dest to the
                    // routing table The first hop taken is the first octet
...
void RHMesh::peekAtMessage(RoutedMessage* message, uint8_t messageLen) {
    ...
    if (messageLen > 1 &&
        ...
        // This is a unicast RH_MESH_MESSAGE_TYPE_ROUTE_DISCOVERY_RESPONSE
        // messages being routed back to the originator here. Want to scrape
        // some routing data out of the response We can find the routes to all
        // the nodes between here and the responding node
        MeshRouteDiscoveryMessage* d =
            (MeshRouteDiscoveryMessage*)message->data;
        addRouteTo(d->dest, headerFrom());
        uint8_t numRoutes = messageLen - sizeof(RoutedMessageHeader) -
                            sizeof(MeshMessageHeader) - 2;
        uint8_t i;
        // Find us in the list of nodes that were traversed to get to the
        // responding node
...
uint8_t RHMesh::route(RoutedMessage* message, uint8_t messageLen) {
    ...
    if (ret == RH_ROUTER_ERROR_NO_ROUTE ||
        ...
        if (message->header.source != _thisAddress) {
            ...
            p->dest =
                message->header.dest;  // Who you were trying to deliver to
...
bool RHMesh::isPhysicalAddress(uint8_t* address, uint8_t addresslen) {
    // Can only handle physical addresses 1 octet long, which is the physical
    // node address
...
bool RHMesh::recvfromAck(uint8_t* buf, uint8_t* len, uint8_t* source,
    ...
    uint8_t _id;
    uint8_t _flags;
    if (RHRouter::recvfromAck(_tmpMessage, &tmpMessageLen, &_source, &_dest,
                              &_id, &_flags)) {
        ...
        if (tmpMessageLen >= 1 &&
            ...
            // Message is an array of node addresses the route request has
            // already passed through If it originally came from us, ignore it
            ...
            if (isPhysicalAddress(&d->dest, d->destlen)) {
                // This route discovery is for us. Unicast the whole route back
                // to the originator as a
                // RH_MESH_MESSAGE_TYPE_ROUTE_DISCOVERY_RESPONSE We are certain
                // to have a route there, because we just got it
                d->header.msgType =
                    RH_MESH_MESSAGE_TYPE_ROUTE_DISCOVERY_RESPONSE;
                RHRouter::sendtoWait((uint8_t*)d, tmpMessageLen, _source);
            } else if (i < _max_hops) {
                // Its for someone else, rebroadcast it, after adding ourselves
                // to the list
