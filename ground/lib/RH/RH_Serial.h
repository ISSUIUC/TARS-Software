...
// header Here we allow for 4 bytes of address and header and payload to be
...
/// It can be used for point-to-point or multidrop, RS232, RS488 or other serial
/// connections as supported by your controller hardware. It can also be used to
/// communicate via radios with serial interfaces such as:
/// - APC220 Radio Data Module
...
/// - Others
///
/// Compiles and runs on Linux, OSX and all the microprocessers and MCUs
/// suported by radiohead. On Linux and OSX, a RadioHead specific version of
/// HardwareSerial (in RHutil/HardwareSerial.*) encapsulates access to any
/// serial port (or suported USB-serial converter)
///
/// The packetised messages include message encapsulation, headers, a message
/// payload and a checksum. It therefore can support robust binary message
/// passing with error-detection and retransmission when used with the
/// appropriate manager. This allows reliable serial communicaitons even over
/// very long lines where noise might otherwise affect reliablity of the
/// communications.
///
/// \par Packet Format
///
/// All messages sent and received by this RH_Serial Driver conform to this
/// packet format: \code DLE STX TO Header                (1 octet) FROM Header
/// (1 octet) ID Header                (1 octet) FLAGS Header             (1
/// octet) Message payload          (0 to 60 octets) DLE ETX Frame Check
/// Sequence FCS CCITT CRC-16 (2 octets) \endcode
///
...
/// The FCS covers everything from the TO header to the ETX inclusive, but not
/// any stuffed DLEs
///
/// \par Physical connection
///
/// The physical connection to your serial port will depend on the type of
/// platform you are on.
///
/// For example, many arduinos only support a single Serial port on pins 0 and
/// 1, which is shared with the USB host connections. On such Arduinos, it is
/// not possible to use both RH_Serial on the Serial port as well as using the
/// Serial port for debugand other printing or communications.
///
/// On Arduino Mega and Due, there are 4 serial ports:
/// - Serial: this is the serial port connected to the USB interface and the
/// programming host.
...
/// - On OSX, names like /dev/tty.usbserial-A501YSWL (for a FTDO USB-serial
/// converter)
///
/// Note that it is necessary for you to select which Serial port your RF_Serial
/// will use and pass it to the contructor. On Linux you must pass an instance
/// of HardwareSerial.
///
/// \par Testing
///
...
/// Back-to-back means the TX of one is connected to the RX of the other and
/// vice-versa. You should also join the ground pins.
///
...
/// \endcode
/// In one window run the server, specifying the device to use as an environment
/// variable: \code RH_HARDWARESERIAL_DEVICE_NAME=/dev/ttyUSB1
/// ./serial_reliable_datagram_server \endcode And in another window run the
/// client, specifying the other device to use as an environment variable: \code
...
class RH_Serial : public RHGenericDriver {
   public:
    /// Constructor
    /// \param[in] serial Reference to the HardwareSerial port which will be
    /// used by this instance. On Unix and OSX, this is an instance of
    /// RHutil/HardwareSerial. On Arduino and other, it is an instance of the
    /// built in HardwareSerial class.
    ...
    /// Wait until a new message is available from the driver or the timeout
    /// expires. Blocks until a complete message is received as reported by
    /// available() or the timeout expires. \param[in] timeout The maximum time
    /// to wait in milliseconds \return true if a message is available as
    /// reported by available(), false on timeout.
   ...
   protected:
    ...
    /// Checks whether the Rx buffer contains valid data that is complete and
    /// uncorrupted Check the FCS, the TO address, and extracts the headers
    void validateRxBuf();
    ...
    /// Progressive FCS calc (CCITT CRC-16 covering all received data (but not
    /// stuffed DLEs), plus trailing DLE, ETX)
    uint16_t _rxFcs;
    ...
    /// True if the data in the Rx buffer is value and uncorrupted and complete
    /// message is available for collection
    bool _rxBufValid;
