...
// The headers are inside the RF69's payload and are therefore encrypted if
// encryption is enabled
#define RH_RF69_HEADER_LEN 4

// This is the maximum message length that can be supported by this driver.
// Limited by the size of the FIFO, since we are unable to support on-the-fly
// filling and emptying of the FIFO. Can be pre-defined to a smaller size (to
// save SRAM) prior to including this header Here we allow for 4 bytes of
...
// You can define the following macro (either by editing here or by passing it
// as a compiler definition to change the default value of the ishighpowermodule
// argument to setTxPower to true
//
// #define RFM69_HW
#ifdef RFM69_HW
...
/// RF69 and compatible radio transceiver.
///
/// Works with
/// - the excellent Moteino and Moteino-USB
/// boards from LowPowerLab http://lowpowerlab.com/moteino/
/// - compatible chips and modules such as RFM69W, RFM69HW, RFM69CW, RFM69HCW
/// (Semtech SX1231, SX1231H),
/// - RFM69 modules from http://www.hoperfusa.com such as
/// http://www.hoperfusa.com/details.jsp?pid=145
/// - Anarduino MiniWireless -CW and -HW boards
/// http://www.anarduino.com/miniwireless/ including
///  the marvellous high powered MinWireless-HW (with 20dBm output for excellent
///  range)
...
///   (https://talk2.wisen.com.au/ and
///   https://bitbucket.org/talk2/whisper-node-avr), an Arduino compatible
///   board, which include an on-board RF69 radio, external antenna, run on
///   2xAAA batteries and support low power operations. RF69 examples work
///   without modification. Use Arduino Board Manager to install the Talk2 code
///   support as described in https://bitbucket.org/talk2/whisper-node-avr
/// - The excellent Adafruit Feather. These are excellent boards that are
/// available with a variety of radios.
///   We tested with the
...
/// data messages with Hope-RF RF69B and compatible radio modules, such as the
/// RFM69 module.
///
...
/// 61Hz precision to any frequency from 240.0MHz to 960.0MHz. Caution: most
/// modules only support a more limited range of frequencies due to antenna
/// tuning.
///
...
/// \par Packet Format
///
/// All messages sent and received by this RH_RF69 Driver conform to this packet
/// format:
///
/// - 4 octets PREAMBLE
...
/// for interrupts).
///
/// If you have a Teensy 3.1 and a compatible RFM69 breakout board, you will
/// need to construct the RH_RF69 instance like this: \code RH_RF69 driver(15,
/// 16); \endcode
///
...
/// (the RFM69 module is soldered onto the MotienoMEGA), but you must initialise
/// the RH_RF69 constructor like this: \code RH_RF69 driver(4, 2); \endcode Make
/// sure you have the MoteinoMEGA core installed in your Arduino hardware folder
/// as described in the documentation for the MoteinoMEGA.
///
...
/// initialise RH_RF69 like this: \code
...
/// intitalisation: \code rf69.setFrequency(915.0); rf69.setTxPower(20);
/// \endcode
...
/// the example rf69_* sketches work without modifications. Initialise the radio
/// like with the default constructor: \code
///  RH_RF69 driver;
/// \endcode
///
/// If you have a Feather 32u4 with RFM69HCW you need to initialise the driver
/// like: \code
///  RH_RF69 driver(8, 7);
/// \endcode
/// and since the radio is the high power HCW model, you must set the Tx power
/// in the range 14 to 20 like this: \code
///  driver.setTxPower(14);
/// \endcode
///
/// If you are connecting an RF69 to a ESP8266 board breakout board that exposes
/// pins 12, 13, 14, 15 (ie NOT an ESP-01) you can connect like this: \code
///                   ESP8266      RFM69W
///                    GND-----------GND   (ground in)
///                    VIN-----------3.3V  (3.3V in)
...
/// master mode.
///
/// Caution: Power supply requirements of the RF69 module may be relevant in
/// some circumstances: RF69 modules are capable of pulling 45mA+ at full power,
/// where Arduino's 3.3V line can give 50mA. You may need to make provision for
/// alternate power supply for the RF69, especially if you wish to use full
/// transmit power, and/or you have other shields demanding power. Inadequate
/// power for the RF69 is likely to cause symptoms such as: -reset's/bootups
/// terminate with "init failed" messages -random termination of communication
/// after 5-30 packets sent/received
...
/// 1/4 wavelength wire antenna. Caution: although the RFM69 appears to have a
/// PC antenna on board, you will get much better power and range even with just
/// a 1/4 wave wire antenna.
///
/// \par Performance
///
...
/// tested with RH_RF69::GFSK_Rb250Fd250, which is probably the fastest scheme
...
/// 13 octet message show about 152 messages per second transmitted and
/// received.
///
/// Transmit-and-wait-for-a-reply tests with modulation RH_RF69::GFSK_Rb250Fd250
/// and a 13 octet message (send and receive) show about 68 round trips per
/// second.
///
class RH_RF69 : public RHSPIDriver {
   public:
    ...
    /// will try to avoid it). CAUTION: some of these configurations do not work
    /// corectly and are marked as such.
    typedef enum {
        ...
        FSK_Rb55555Fd50,   ///< FSK, Whitening, Rb = 55555kbs,Fd = 50kHz for
                           ///< RFM69 lib compatibility
        ...
        OOK_Rb4_8Bw9_6,    ///< OOK, Whitening, Rb = 4.8kbs,  Rx Bandwidth
                           ///< = 9.6kHz.
        OOK_Rb9_6Bw19_2,   ///< OOK, Whitening, Rb = 9.6kbs,  Rx Bandwidth
                           ///< = 19.2kHz.
    ...
    /// RF69 before accessing it. Defaults to the normal SS pin for your Arduino
    /// (D10 for Diecimila, Uno etc, D53 for Mega, D10 for Maple) \param[in]
    /// interruptPin The interrupt Pin number that is connected to the RF69 DIO0
    ...
    /// pin. On Arduino Zero from arduino.cc, any digital pin other than 4. On
    /// Arduino M0 Pro from arduino.org, any digital pin other than 2. On other
    ...
    /// The measurement is uncalibrated and without calibration, you can expect
    /// it to be far from correct. \return The measured temperature, in degrees
    /// C from -40 to 85 (uncalibrated)
    int8_t temperatureRead();

    /// Sets the transmitter and receiver
    /// centre frequency
    /// \param[in] centre Frequency in MHz. 240.0 to 960.0. Caution, RF69 comes
    /// in several different frequency ranges, and setting a frequency outside
    /// that range of your radio will probably not work \param[in]
    /// afcPullInRange Not used \return true if the selected frquency centre is
    /// within range
    ...
    /// This is a low level device access function, and should not normally ned
    /// to be used by user code. Instead can use stModeRx(), setModeTx(),
    /// setModeIdle() \param[in] mode RF69 OPMODE to set, one of
    /// RH_RF69_OPMODE_MODE_*.
    ...
    /// If you are using a high p[ower modfule such as an RFM69HW, you MUST set
    /// the power level with the ishighpowermodule flag set to true. Else you
    /// wil get no measurable power output. Simlarly if you are not using a high
    /// power module, you must NOT set the ishighpowermodule (which is the
    /// default) \param[in] power Transmitter power level in dBm. For RF69W
    /// (ishighpowermodule = false), valid values are from -18 to +13.; Values
    /// outside this range are trimmed. For RF69HW (ishighpowermodule = true),
    /// valid values are from -2 to +20. Caution: at +20dBm, duty cycle is
    ...
    /// the modem with custom configurations if none of the canned
    ...
    /// ModemConfig. The default after init() is RH_RF69::GFSK_Rb250Fd250.
    ...
    /// 0xd4 }. Caution: tests here show that with a single sync word (ie where
    /// len == 1), RFM69 reception can be unreliable. \param[in] syncWords Array
    /// of sync words, 1 to 4 octets long. NULL if no sync words to be used.
    /// \param[in] len Number of sync words to set, 1 to 4. 0 if no sync words
    /// to be used.
    ...
    /// \param[in] key The key to use. Must be 16 bytes long. The same key must
    /// be installed in other instances of RF69, otherwise communications will
    /// not work correctly. If key is NULL, encryption is disabled, which is the
    /// default.
    void setEncryptionKey(uint8_t* key = NULL);

    /// Returns the time in millis since the most recent preamble was received,
    /// and when the most recent RSSI measurement was made.
    ...
    /// platform For debugging/testing only \return true if successful
    ...
    /// receive modes. By default, the idle mode is RH_RF69_OPMODE_MODE_STDBY,
    ...
    /// \param[in] idleMode The chip operating mode to use when the driver is
    /// idle. One of RH_RF69_OPMODE_*
   ...
   protected:
    ...
    /// of RF69. Called automatically by isr*() Should not need to be called by
    /// user code.
    void handleInterrupt();

    /// Low level function to read the FIFO and put the received data into the
    /// receive buffer Should not need to be called by user code.
    void readFifo();
   ...
   protected:
    ...
    /// Array of octets of teh last received message or the next to transmit
    /// message
