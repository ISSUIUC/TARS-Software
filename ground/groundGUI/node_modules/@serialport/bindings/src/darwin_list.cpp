...
static io_service_t GetUsbDevice(io_service_t service) {
    ...
    if (status == kIOReturnSuccess) {
        ...
        while ((currentService = IOIteratorNext(iterator)) && device == 0) {
            ...
            status = IORegistryEntryGetNameInPlane(
                currentService, kIOServicePlane, serviceName);
            if (status == kIOReturnSuccess &&
...
static void ExtractUsbInformation(stSerialDevice* serialDevice,
    ...
    if (KERN_SUCCESS == kernResult) {
        snprintf(serialDevice->locationId, sizeof(serialDevice->locationId),
                 "%08x", locationID);
    }
    ...
    if (KERN_SUCCESS == kernResult) {
        snprintf(serialDevice->vendorId, sizeof(serialDevice->vendorId), "%04x",
                 vendorID);
    }
    ...
    if (KERN_SUCCESS == kernResult) {
        snprintf(serialDevice->productId, sizeof(serialDevice->productId),
                 "%04x", productID);
    }
}
    ...
    *bsdPath = '\0';
    ...
    while ((modemService = IOIteratorNext(serialPortIterator))) {
        ...
        if (bsdPathAsCFString) {
            ...
            if (result) {
                stDeviceListItem* deviceListItem =
                    reinterpret_cast<stDeviceListItem*>(
                        malloc(sizeof(stDeviceListItem)));
                stSerialDevice* serialDevice = &(deviceListItem->value);
                snprintf(serialDevice->port, sizeof(serialDevice->port), "%s",
                         bsdPath);
                memset(serialDevice->locationId, 0,
                       sizeof(serialDevice->locationId));
                memset(serialDevice->vendorId, 0,
                       sizeof(serialDevice->vendorId));
                memset(serialDevice->productId, 0,
                       sizeof(serialDevice->productId));
                ...
                if (device) {
                    ...
                    CFStringRef serialNumberAsCFString =
                        (CFStringRef)IORegistryEntrySearchCFProperty(
                            ...
                            CFSTR(kUSBSerialNumberString), kCFAllocatorDefault,
                    ...
                    if ((kIOReturnSuccess == kernResult) && plugInInterface) {
                        // Use the plugin interface to retrieve the device
                        // interface.
                        res =
                            (*plugInInterface)
                                ->QueryInterface(
                        ...
                        if (!res && deviceInterface != NULL) {
                            ...
                            ExtractUsbInformation(serialDevice,
                                                  deviceInterface);
