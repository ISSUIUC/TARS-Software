...
void EIO_Open(uv_work_t* req) {
    ...
    if (-1 == fd) {
        snprintf(data->errorString, sizeof(data->errorString),
                 "Error: %s, cannot open %s", strerror(errno), data->path);
        return;
    }
...
int setBaudRate(ConnectionOptions* data) {
    ...
    if (-1 == tcgetattr(fd, &options)) {
        snprintf(data->errorString, sizeof(data->errorString),
                 "Error: %s setting custom baud rate of %d", strerror(errno),
                 data->baudRate);
        return -1;
    }

// If there is a custom baud rate on linux you can do the following trick with
// B38400
    ...
    if (baudRate == -1) {
        ...
        if (err == -1) {
            snprintf(data->errorString, sizeof(data->errorString),
                     "Error: %s || while retrieving termios2 info",
                     strerror(errno));
            return -1;
        } else if (err == -2) {
            snprintf(data->errorString, sizeof(data->errorString),
                     "Error: %s || while setting custom baud rate of %d",
                     strerror(errno), data->baudRate);
            return -1;
        }
...
// On OS X, starting with Tiger, we can set a custom baud rate with ioctl
    ...
    if (-1 == baudRate) {
        ...
        if (-1 == ioctl(fd, IOSSIOSPEED, &speed)) {
            snprintf(data->errorString, sizeof(data->errorString),
                     "Error: %s calling ioctl(.., IOSSIOSPEED, %ld )",
    ...
    if (-1 == baudRate) {
        snprintf(data->errorString, sizeof(data->errorString),
                 "Error baud rate of %d is not supported on your platform",
                 data->baudRate);
        return -1;
    }
...
int setup(int fd, OpenBaton* data) {
    ...
    // terminate input) Future potential option options.c_iflag = ICRNL;
    ...
    switch (data->parity) {
        ...
        default:
            snprintf(data->errorString, sizeof(data->errorString),
                     "Invalid parity setting %d", data->parity);
            return -1;
    }
    ...
    switch (data->stopBits) {
        ...
        default:
            snprintf(data->errorString, sizeof(data->errorString),
                     "Invalid stop bits setting %d", data->stopBits);
            return -1;
    }
    ...
    if (data->lock) {
        if (-1 == flock(fd, LOCK_EX | LOCK_NB)) {
            snprintf(data->errorString, sizeof(data->errorString),
                     "Error %s Cannot lock port", strerror(errno));
            return -1;
        }
    }

    // Copy the connection options into the ConnectionOptionsBaton to set the
    // baud rate
    ...
    if (-1 == setBaudRate(connectionOptions)) {
        strncpy(data->errorString, connectionOptions->errorString,
                sizeof(data->errorString));
        delete (connectionOptions);
        return -1;
    }
    delete (connectionOptions);

    // flush all unread and wrote data up to this point because it could have
    // been received or sent with bad settings Not needed since setBaudRate does
    // this for us tcflush(fd, TCIOFLUSH);
...
void EIO_Set(uv_work_t* req) {
    ...
    if (-1 == result) {
        snprintf(data->errorString, sizeof(data->errorString),
                 "Error: %s, cannot set", strerror(errno));
        return;
    }
    ...
    // Only report errors when the lowLatency is being set to true.  Attempting
    // to set as false can error, since the default is false
    if (data->lowLatency) {
        if (err == -1) {
            snprintf(data->errorString, sizeof(data->errorString),
                     "Error: %s, cannot get low latency", strerror(errno));
            return;
        } else if (err == -2) {
            snprintf(data->errorString, sizeof(data->errorString),
                     "Error: %s, cannot set low latency", strerror(errno));
            return;
        }
    }
#endif
...
void EIO_Get(uv_work_t* req) {
    ...
    if (-1 == ioctl(data->fd, TIOCMGET, &bits)) {
        snprintf(data->errorString, sizeof(data->errorString),
                 "Error: %s, cannot get", strerror(errno));
        return;
    }
    ...
    // Try to get low latency info, but we don't care if fails (a failure state
    // will still return lowlatency = false)
...
void EIO_GetBaudRate(uv_work_t* req) {
    ...
    if (-1 == linuxGetSystemBaudRate(data->fd, &outbaud)) {
        snprintf(data->errorString, sizeof(data->errorString),
                 "Error: %s, cannot get baud rate", strerror(errno));
        return;
    }
#else
    snprintf(data->errorString, sizeof(data->errorString),
             "Error: System baud rate check not implemented on this platform");
    return;
#endif
...
void EIO_Flush(uv_work_t* req) {
    ...
    if (-1 == tcflush(data->fd, TCIOFLUSH)) {
        snprintf(data->errorString, sizeof(data->errorString),
                 "Error: %s, cannot flush", strerror(errno));
        return;
    }
}
...
void EIO_Drain(uv_work_t* req) {
    ...
    if (-1 == tcdrain(data->fd)) {
        snprintf(data->errorString, sizeof(data->errorString),
                 "Error: %s, cannot drain", strerror(errno));
        return;
    }
}
