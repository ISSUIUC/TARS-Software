...
static lzma_ret parse_indexes_read(lzma_index_parser_data* info, uint8_t* buf,
...
extern lzma_ret my_lzma_parse_indexes_from_file(lzma_index_parser_data* info) {
    lzma_ret ret;
    ...
    if (internal == NULL) {
        ...
        internal = (lzma_index_parser_internal*)lzma_alloc(
            sizeof(lzma_index_parser_internal), info->allocator);

        if (internal == NULL) return LZMA_MEM_ERROR;
        ...
        case PARSE_INDEX_INITED:
            ...
            do {
                ...
                while (true) {
                    ...
                    ret = parse_indexes_read(info, internal->buf,
                                             LZMA_STREAM_HEADER_SIZE,
                                             internal->pos);

                    if (ret != LZMA_OK) goto error;
                    ...
                    case PARSE_INDEX_READ_FOOTER:
                        ...
                        i = 2;
                        if (((uint32_t*)internal->buf)[i] != 0) break;
                ...
                if ((lzma_vli)(internal->pos) <
                    internal->index_size + LZMA_STREAM_HEADER_SIZE) {
                    ret = LZMA_DATA_ERROR;
                    goto error;
                }
                ...
                if (internal->combined_index != NULL) {
                    internal->memused =
                        lzma_index_memused(internal->combined_index);
                ...
                // Decode the Index.
                ret = lzma_index_decoder(&internal->strm, &internal->this_index,
                                         memlimit);
                if (ret != LZMA_OK) {
                    goto error;
                }
                ...
                do {
                    ...
                    internal->strm.avail_in =
                        my_min(sizeof(internal->buf), internal->index_size);
                ...
                if (ret != LZMA_STREAM_END) {
                    ...
                    // sense in that case.
                    if (ret == LZMA_BUF_ERROR) ret = LZMA_DATA_ERROR;
                ...
                ret =
                    parse_indexes_read(info, internal->buf,
                                       LZMA_STREAM_HEADER_SIZE, internal->pos);
                ...
                case PARSE_INDEX_READ_STREAM_HEADER:
                    ...
                    ret =
                        lzma_stream_header_decode(&header_flags, internal->buf);
                    if (ret != LZMA_OK) {
                        goto error;
                    }
                    ...
                    if (internal->combined_index != NULL) {
                        ...
                        // after this_index.
                        ret = lzma_index_cat(internal->this_index,
...
error:
    ...
    // Doing this will prevent people from calling
    // lzma_parse_indexes_from_file() again without re-initializing.
...
void IndexParser::InitializeExports(Object exports) {
    exports["IndexParser"] =
        DefineClass(exports.Env(), "IndexParser",
                    {
...
namespace {
extern "C" int64_t LZMA_API_CALL read_cb(void* opaque, uint8_t* buf,
                                         size_t count, int64_t offset) {
    IndexParser* p = static_cast<IndexParser*>(opaque);
    return p->readCallback(opaque, buf, count, offset);
}

extern "C" void* LZMA_API_CALL alloc_for_lzma_index(void* opaque, size_t nmemb,
                                                    size_t size) {
    IndexParser* p = static_cast<IndexParser*>(opaque);
    ...
    *result = nBytes;
    MemoryManagement::AdjustExternalMemory(p->Env(),
                                           static_cast<int64_t>(nBytes));
    return static_cast<void*>(result + 1);
}

extern "C" void LZMA_API_CALL free_for_lzma_index(void* opaque, void* ptr) {
    IndexParser* p = static_cast<IndexParser*>(opaque);
    if (!ptr) return;
...
int64_t IndexParser::readCallback(void* opaque, uint8_t* buf, size_t count,
                                  int64_t offset) {
    currentReadBuffer = buf;
    currentReadSize = count;

    napi_value argv[2] = {Uint64ToNumberMaxNull(Env(), count),
...
IndexParser::IndexParser(const CallbackInfo& args)
    : ObjectWrap(args), isCurrentlyInParseCall(false) {
...
Object IndexParser::getObject() const {
    ...
    obj["streams"] =
        Uint64ToNumberMaxNull(env, lzma_index_stream_count(info.index));
    obj["blocks"] =
        Uint64ToNumberMaxNull(env, lzma_index_block_count(info.index));
    obj["fileSize"] =
        Uint64ToNumberMaxNull(env, lzma_index_file_size(info.index));
    obj["uncompressedSize"] =
        Uint64ToNumberMaxNull(env, lzma_index_uncompressed_size(info.index));
...
Value IndexParser::Parse(const CallbackInfo& args) {
    ...
    struct RecursionGuard {
        ...
        ~RecursionGuard() { p->isCurrentlyInParseCall = false; }
        IndexParser* p;
    };
...
Value IndexParser::Feed(const CallbackInfo& info) {
    ...
    if (length > currentReadSize) length = currentReadSize;

    memcpy(currentReadBuffer,
           static_cast<const uint8_t*>(value.ArrayBuffer().Data()) +
               value.ByteOffset(),
           length);
