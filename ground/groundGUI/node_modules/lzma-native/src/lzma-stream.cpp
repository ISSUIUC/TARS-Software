...
namespace {
extern "C" void* LZMA_API_CALL alloc_for_lzma(void* opaque, size_t nmemb,
                                              size_t size) {
...
extern "C" void LZMA_API_CALL free_for_lzma(void* opaque, void* ptr) {
    LZMAStream* strm = static_cast<LZMAStream*>(opaque);
...
void LZMAStream::resetUnderlying() {
    if (_.internal != nullptr) lzma_end(&_);
...
Value LZMAStream::SetBufsize(const CallbackInfo& info) {
...

    {
        ...
        if (newBufsize && newBufsize != UINT_MAX) bufsize = newBufsize;
    }
...
void LZMAStream::Code(const CallbackInfo& info) {
    ...
    if (info[0].IsUndefined() || info[0].IsNull()) {
        ...
        if (!readBufferFromObj(info[0], &inputData)) return;

        if (inputData.empty()) shouldFinish = true;
    }
...
void LZMAStream::invokeBufferHandlers(bool hasLock) {
    ...
    if (!hasLock) lock = std::unique_lock<std::mutex>(mutex);

    Function bufferHandler =
        Napi::Value(Value()["bufferHandler"]).As<Function>();
    ...
    if (_.internal) lzma_get_progress(&_, &in, &out);
    ...
    if (processedChunks) {
        ...
        napi_value argv[5] = {env.Undefined(),
                              Number::New(env, static_cast<uint32_t>(pc)),
                              env.Undefined(), in_, out_};
        CallBufferHandlerWithArgv(5, argv);
    }

    if (reset) resetUnderlying();  // resets lastCodeResult!
}
...
void LZMAStream::doLZMACode() {
    ...
    // _.internal is set to nullptr when lzma_end() is called via
    // resetUnderlying()
    while (_.internal) {
        ...
        if (shouldFinish && inbufs.empty()) action = LZMA_FINISH;
        ...
        if (_.avail_out == 0 || _.avail_in == 0 ||
            lastCodeResult == LZMA_STREAM_END) {
...
#else
                outbufs.push(
                    std::vector<uint8_t>(outbuf.data(), outbuf.data() + outsz));
#endif
            }
...
void LZMAStream::InitializeExports(Object exports) {
    exports["Stream"] = DefineClass(
        exports.Env(), "LZMAStream",
        {
...
Value LZMAStream::MemlimitSet(const CallbackInfo& info) {
    ...
    return lzmaRet(Env(),
                   lzma_memlimit_set(&_, NumberToUint64ClampNullMax(arg)));
}
...
Value LZMAStream::StreamEncoder(const CallbackInfo& info) {
    ...
    return lzmaRet(Env(),
                   lzma_stream_encoder(&_, filters.array(), (lzma_check)check));
}
