...
Error lzmaRetError(Env env, lzma_ret rv) {
    ...
    while (p->code != rv && p->code != (lzma_ret)-1) ++p;
...
Number lzmaRet(Env env, lzma_ret rv) {
    if (rv != LZMA_OK && rv != LZMA_STREAM_END) throw lzmaRetError(env, rv);
...
bool readBufferFromObj(Value buf_, std::vector<uint8_t>* data) {
    ...
    const uint8_t* ptr =
        len > 0 ? static_cast<const uint8_t*>(buf.ArrayBuffer().Data()) +
                      buf.ByteOffset()
...
lzma_options_lzma parseOptionsLZMA(Value val) {
    ...
    Object obj = val.IsUndefined() || val.IsNull() ? Object::New(val.Env())
                                                   : val.ToObject();
    ...
    r.mode =
        (lzma_mode)GetIntegerProperty(obj, "mode", (int64_t)LZMA_MODE_FAST);
    r.nice_len = GetIntegerProperty(obj, "niceLen", 64);
    r.mf =
        (lzma_match_finder)GetIntegerProperty(obj, "mf", (int64_t)LZMA_MF_HC4);
    r.depth = GetIntegerProperty(obj, "depth", 0);
    uint64_t preset_ = GetIntegerProperty(obj, "preset", UINT64_MAX);

    r.preset_dict = nullptr;

    if (preset_ != UINT64_MAX) lzma_lzma_preset(&r, preset_);
...
uint64_t NumberToUint64ClampNullMax(Value in) {
    if (in.IsNull() || in.IsUndefined()) return UINT64_MAX;
