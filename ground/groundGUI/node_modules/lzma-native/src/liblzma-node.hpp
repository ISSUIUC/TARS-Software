...
/**
 * Return a v8 Number representation of an uint64_t where UINT64_MAX will be
 * mapped to null
 */
Value Uint64ToNumberMaxNull(Env env, uint64_t in);

/**
 * Return a v8 Number representation of an uint64_t where 0 will be mapped to
 * null
 */
...
class LZMAStreamCodingWorker : public AsyncWorker {
   public:
    LZMAStreamCodingWorker(LZMAStream* stream_)
        : AsyncWorker(Function(stream_->Env(), nullptr),
    ...
    void Execute() override { stream->doLZMACodeFromAsync(); }

   private:
    void OnOK() { stream->invokeBufferHandlers(false); }

    void OnOK(const Error& e) { stream->invokeBufferHandlers(false); }
...
class IndexParser : public ObjectWrap<IndexParser> {
   public:
    ...
    /* regard as private: */
    int64_t readCallback(void* opaque, uint8_t* buf, size_t count,
                         int64_t offset);
