...
FilterArray::FilterArray(Value val) {
    ...
    if (!val.IsArray()) throw TypeError::New(env, "Filter array expected");
    ...
    for (size_t i = 0; i < len; ++i) {
        ...
        switch (f.id) {
            case LZMA_FILTER_DELTA:
                bopt.delta.type = (lzma_delta_type)GetIntegerProperty(
            ...
            default:
                throw TypeError::New(
                    env,
                    "LZMA wrapper library understands .options only for DELTA "
                    "and LZMA1, LZMA2 filters");
        }
