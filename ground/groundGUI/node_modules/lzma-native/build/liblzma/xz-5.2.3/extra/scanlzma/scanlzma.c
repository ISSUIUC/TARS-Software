...
/* Usage example:
   ...
   $ ./scanlzma 0 < WA2204-FW1.4.1/linux-1.4.bin | lzma -c -d | strings | grep
   -i "copyright" UpdateDD version 2.5, Copyright (C) 2005 Philipp Benner.
...
int find_lzma_header(unsigned char *buf) {
    return (buf[0] < 0xE1 && buf[0] == 0x5d && buf[4] < 0x20 &&
...
int main(int argc, char *argv[]) {
    ...
    if (argc != 2) {
        printf(
            ...
            "where numlzma is index of lzma file to extract, starting from "
            "zero.\n",
            argv[0]);
        exit(1);
    }
