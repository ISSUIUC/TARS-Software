...
extern void message_init(void) {
    ...
    /*
            if (progress_automatic) {
                    ...
                    // variable to see if the terminal is wide enough. If
       COLUMNS
                    ...
                    if (columns_str != NULL) {
                            char *endptr;
                            const long columns = strtol(columns_str, &endptr,
       10); if (*endptr != '\0' || columns < 80) progress_automatic = false;
                    }
            }
    */
...
extern void message_verbosity_increase(void) {
    if (verbosity < V_DEBUG) ++verbosity;

    return;
}

extern void message_verbosity_decrease(void) {
    if (verbosity > V_SILENT) --verbosity;

    return;
}

extern enum message_verbosity message_verbosity_get(void) { return verbosity; }
...
static void print_filename(void) {
    if (!opt_robot && (files_total != 1 || filename != stdin_filename)) {
        ...
        if (first_filename_printed) fputc('\n', file);
...
static const char *progress_percentage(uint64_t in_pos) {
    ...
    if (expected_in_size == 0 || in_pos > expected_in_size) return "--- %";
...
static const char *progress_speed(uint64_t uncompressed_pos, uint64_t elapsed) {
    ...
    while (speed > 999.0) {
        speed /= 1024.0;
        if (++unit_index == ARRAY_SIZE(unit)) return "";  // Way too fast ;-)
    }
...
static const char *progress_time(uint64_t mseconds) {
    ...
    if (seconds == 0 || seconds > ((9999 * 60) + 59) * 60 + 59) return "";
...
extern void message_progress_update(void) {
    if (!progress_needs_updating) return;

    // Calculate how long we have been processing this file.
    const uint64_t elapsed = mytime_get_elapsed();

#ifndef SIGALRM
    if (progress_next_update > elapsed) return;
    ...
    if (!current_filename_printed) print_filename();
...
static void progress_flush(bool finished) {
    if (!progress_started || verbosity < V_VERBOSE) return;
    ...
    if (progress_automatic) {
        ...
        if (!finished) {
            ...
            if (percentage[0] != '-') fprintf(stderr, "%s, ", percentage);
        }
        ...
        if (speed[0] != '\0') fprintf(stderr, ", %s", speed);

        const char *elapsed_str = progress_time(elapsed);
        if (elapsed_str[0] != '\0') fprintf(stderr, ", %s", elapsed_str);
...
extern void message_bug(void) { message_fatal(_("Internal error (bug)")); }
...
extern void message_help(bool long_help) {
    ...
    if (long_help)
        puts(
            _("Mandatory arguments to long options are mandatory "
              "for short options too.\n"));

    if (long_help) puts(_(" Operation mode:\n"));

    puts(
        _("  -z, --compress      force compression\n"
    ...
    puts(
        _("  -k, --keep          keep (don't delete) input files\n"
    ...
    if (long_help) {
        puts(
            _("      --single-stream decompress only the first stream, and "
              "silently\n"
    ...
    if (long_help) {
        ...
        puts(
            _("      --ignore-check  don't verify the integrity check when "
              "decompressing"));
    }

    puts(
        _("  -0 ... -9           compression preset; default is 6; take "
          "compressor *and*\n"
          "                      decompressor memory usage into account before "
          "using 7-9!"));

    puts(
        _("  -e, --extreme       try to improve compression ratio by using "
          "more CPU time;\n"
          "                      does not affect decompressor memory "
          "requirements"));

    puts(
        _("  -T, --threads=NUM   use at most NUM threads; the default is 1; "
          "set to 0\n"
          "                      to use as many threads as there are processor "
          "cores"));
    ...
    if (long_help) {
        ...
        puts(
            _("      --no-adjust     if compression settings exceed the memory "
              "usage limit,\n"
    ...
    defined(HAVE_ENCODER_LZMA2) || defined(HAVE_DECODER_LZMA2)
        ...
        puts(
            _("\n"
              "  --lzma1[=OPTS]      LZMA1 or LZMA2; OPTS is a comma-separated "
              "list of zero or\n"
              ...
              "(2-273; 64)\n"
              "                        mf=NAME    match finder (hc3, hc4, bt2, "
              "bt3, bt4; bt4)\n"
    ...
    puts(
        _("  -q, --quiet         suppress warnings; specify twice to suppress "
          "errors too\n"
    ...
    if (long_help) {
        ...
        puts(
            _("      --robot         use machine-parsable messages (useful for "
              "scripts)"));
        puts("");
        puts(
            _("      --info-memory   display the total amount of RAM and the "
              "currently active\n"
              "                      memory usage limits, and exit"));
        puts(
            _("  -h, --help          display the short help (lists only the "
              "basic options)\n"
              "  -H, --long-help     display this long help and exit"));
    } else {
        puts(
            _("  -h, --help          display this short help and exit\n"
