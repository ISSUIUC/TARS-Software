...
extern LZMA_API(lzma_ret)
    ...
    if (src == NULL || dest == NULL) return LZMA_PROG_ERROR;
...
static lzma_ret validate_chain(const lzma_filter *filters, size_t *count) {
    ...
    do {
        ...
        for (j = 0; filters[i].id != features[j].id; ++j)
            if (features[j].id == LZMA_VLI_UNKNOWN) return LZMA_OPTIONS_ERROR;

        // If the previous filter in the chain cannot be a non-last
        // filter, the chain is invalid.
        if (!non_last_ok) return LZMA_OPTIONS_ERROR;
...
extern lzma_ret lzma_raw_coder_init(lzma_next_coder *next,
    ...
    if (is_encoder) {
        for (size_t i = 0; i < count; ++i) {
            ...
            if (fc == NULL || fc->init == NULL) return LZMA_OPTIONS_ERROR;
    ...
    if (ret != LZMA_OK) lzma_next_end(next, allocator);
...
extern uint64_t lzma_raw_coder_memusage(lzma_filter_find coder_find,
    ...
    {
        size_t tmp;
        if (validate_chain(filters, &tmp) != LZMA_OK) return UINT64_MAX;
    }
    ...
    do {
        ...
        if (fc == NULL) return UINT64_MAX;  // Unsupported Filter ID
        ...
        if (fc->memusage == NULL) {
            ...
            if (usage == UINT64_MAX) return UINT64_MAX;  // Invalid options
