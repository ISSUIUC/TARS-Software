...
static uint64_t mytime_now(void) {
    ...
    struct timespec tv;
    while (clock_gettime(clk_id, &tv)) clk_id = CLOCK_REALTIME;
...
extern uint64_t mytime_get_elapsed(void) { return mytime_now() - start_time; }
...
extern int mytime_get_flush_timeout(void) {
    if (opt_flush_timeout == 0 || opt_mode != MODE_COMPRESS) return -1;

    const uint64_t now = mytime_now();
    if (now >= next_flush) return 0;
