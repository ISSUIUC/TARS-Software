...
static lzma_ret simple_code(void *coder_ptr, const lzma_allocator *allocator,
    ...
    if (action == LZMA_SYNC_FLUSH) return LZMA_OPTIONS_ERROR;
    ...
    if (coder->pos < coder->filtered) {
        ...
        if (coder->pos < coder->filtered) return LZMA_OK;
    ...
    if (coder->size > 0) {
        ...
        if (coder->end_was_reached) coder->filtered = coder->size;
...
extern lzma_ret lzma_simple_coder_init(
    ...
    if (coder == NULL) {
        ...
        if (simple_size > 0) {
            ...
            if (coder->simple == NULL) return LZMA_MEM_ERROR;
        } else {
            coder->simple = NULL;
        }
    }
    ...
    if (filters[0].options != NULL) {
        ...
        if (coder->now_pos & (alignment - 1)) return LZMA_OPTIONS_ERROR;
    } else {
        coder->now_pos = 0;
    }
