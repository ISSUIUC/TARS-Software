...
static bool lz_encoder_prepare(lzma_mf *mf, const lzma_allocator *allocator,
    ...
    if (hash_bytes > mf->nice_len) return true;
...
extern uint64_t lzma_lz_encoder_memusage(const lzma_lz_options *lz_options) {
    ...
    if (lz_encoder_prepare(&mf, NULL, lz_options)) return UINT64_MAX;
...
static lzma_ret lz_encoder_update(
    ...
    if (coder->lz.options_update == NULL) return LZMA_PROG_ERROR;
...
#ifdef HAVE_MF_HC3
    if (mf == LZMA_MF_HC3) ret = true;
#endif

#ifdef HAVE_MF_HC4
    if (mf == LZMA_MF_HC4) ret = true;
#endif

#ifdef HAVE_MF_BT2
    if (mf == LZMA_MF_BT2) ret = true;
#endif

#ifdef HAVE_MF_BT3
    if (mf == LZMA_MF_BT3) ret = true;
#endif

#ifdef HAVE_MF_BT4
    if (mf == LZMA_MF_BT4) ret = true;
#endif
