...
extern void hardware_threads_set(uint32_t n) {
    if (n == 0) {
        ...
        if (threads_max == 0) threads_max = 1;
#else
        threads_max = 1;
#endif
    } else {
...
extern uint32_t hardware_threads_get(void) { return threads_max; }
...
extern void hardware_memlimit_set(uint64_t new_memlimit, bool set_compress,
    ...
    if (set_compress) memlimit_compress = new_memlimit;

    if (set_decompress) memlimit_decompress = new_memlimit;
...
extern void hardware_init(void) {
    ...
    if (total_ram == 0) total_ram = (uint64_t)(ASSUME_RAM)*1024 * 1024;
