...
static bool parse_indexes(xz_file_info *xfi, file_pair *pair) {
    ...
    do {
        ...
        while (true) {
            ...
            if (io_pread(pair, &buf, LZMA_STREAM_HEADER_SIZE, pos)) goto error;

            // Stream Padding is always a multiple of four bytes.
            int i = 2;
            if (buf.u32[i] != 0) break;
        ...
        if (combined_index != NULL) {
            ...
            if (memused > memlimit) message_bug();
        ...
        do {
            ...
            if (io_pread(pair, &buf, strm.avail_in, pos)) goto error;
        ...
        if (ret == LZMA_STREAM_END)
            if (index_size != 0 || strm.avail_in != 0) ret = LZMA_DATA_ERROR;
...
#error IO_BUFFER_SIZE < LZMA_BLOCK_HEADER_SIZE_MAX
    ...
    const uint32_t size = my_min(
        iter->block.total_size - lzma_check_size(iter->stream.flags->check),
    ...
    if (block.header_size > size) goto data_error;
    ...
    if (xfi->memusage_max < bhi->memusage) xfi->memusage_max = bhi->memusage;
    ...
    {
        size_t i = 0;
        while (filters[i + 1].id != LZMA_VLI_UNKNOWN) ++i;
...
static bool parse_check_value(file_pair *pair, const lzma_index_iter *iter) {
    ...
    io_buf buf;
    if (io_pread(pair, &buf, size, offset)) return true;
...
static bool parse_details(file_pair *pair, const lzma_index_iter *iter,
                          block_header_info *bhi, xz_file_info *xfi) {
    if (parse_block_header(pair, iter, bhi, xfi)) return true;

    if (parse_check_value(pair, iter)) return true;
...
static bool print_info_adv(xz_file_info *xfi, file_pair *pair) {
    ...
    if (lzma_index_block_count(xfi->idx) > 0) {
        ...
        while (!lzma_index_iter_next(&iter, LZMA_INDEX_ITER_BLOCK)) {
            if (detailed && parse_details(pair, &iter, &bhi, xfi)) return true;
