...
static lzma_ret index_decode(
    ...
    while (*in_pos < in_size) switch (coder->sequence) {
            case SEQ_INDICATOR:
                ...
                if (in[(*in_pos)++] != 0x00) return LZMA_DATA_ERROR;
            ...
            case SEQ_COUNT:
                ret = lzma_vli_decode(&coder->count, &coder->pos, in, in_pos,
                                      in_size);
                if (ret != LZMA_STREAM_END) goto out;
...
static lzma_ret index_decoder_memconfig(void *coder_ptr, uint64_t *memusage,
    ...
    *old_memlimit = coder->memlimit;
    ...
    if (new_memlimit != 0) {
        if (new_memlimit < *memusage) return LZMA_MEMLIMIT_ERROR;
...
static lzma_ret index_decoder_init(lzma_next_coder *next,
    ...
    if (i == NULL || memlimit == 0) return LZMA_PROG_ERROR;
