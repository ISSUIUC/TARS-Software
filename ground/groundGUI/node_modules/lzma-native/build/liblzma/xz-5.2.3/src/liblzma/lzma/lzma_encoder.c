...
static bool encode_init(lzma_lzma1_encoder *coder, lzma_mf *mf) {
    ...
    if (mf->read_pos == mf->read_limit) {
        if (mf->action == LZMA_RUN) return false;  // We cannot do anything.
...
extern lzma_ret lzma_lzma_encode(lzma_lzma1_encoder *restrict coder,
    ...
    while (true) {
        ...
        if (mf->read_pos >= mf->read_limit) {
            if (mf->action == LZMA_RUN) return LZMA_OK;
    ...
    if (!coder->is_flushed) {
        ...
        if (limit == UINT32_MAX) encode_eopm(coder, position);
...
static lzma_ret lzma_encode(void *coder, lzma_mf *restrict mf,
    ...
    if (unlikely(mf->action == LZMA_SYNC_FLUSH)) return LZMA_OPTIONS_ERROR;
...
extern uint64_t lzma_lzma_encoder_memusage(const void *options) {
    if (!is_options_valid(options)) return UINT64_MAX;
...
extern bool lzma_lzma_lclppb_encode(const lzma_options_lzma *options,
                                    uint8_t *byte) {
    if (!is_lclppb_valid(options)) return true;
...
extern lzma_ret lzma_lzma_props_encode(const void *options, uint8_t *out) {
    ...
    if (lzma_lzma_lclppb_encode(opt, out)) return LZMA_PROG_ERROR;
