...
static void worker_error(worker_thread *thr, lzma_ret ret) {
    ...
    mythread_sync(thr->coder->mutex) {
        if (thr->coder->thread_error == LZMA_OK) thr->coder->thread_error = ret;
...
static worker_state worker_encode(worker_thread *thr, worker_state state) {
    ...
    do {
        ...
        if (state >= THR_STOP) return state;
...
static MYTHREAD_RET_TYPE worker_start(void *thr_ptr) {
    ...
    while (true) {
        ...
        if (state <= THR_FINISH) state = worker_encode(thr, state);

        if (state == THR_EXIT) break;
...
static lzma_ret initialize_new_thread(lzma_stream_coder *coder,
    ...
    if (thr->in == NULL) return LZMA_MEM_ERROR;

    if (mythread_mutex_init(&thr->mutex)) goto error_mutex;

    if (mythread_cond_init(&thr->cond)) goto error_cond;
    ...
    if (mythread_create(&thr->thread_id, &worker_start, thr)) goto error_thread;
...
static lzma_ret get_thread(lzma_stream_coder *coder,
    ...
    if (!lzma_outq_has_buf(&coder->outq)) return LZMA_OK;
    ...
    if (coder->thr == NULL) {
        ...
        if (coder->threads_initialized == coder->threads_max) return LZMA_OK;
    ...
    while (*in_pos < in_size || (coder->thr != NULL && action != LZMA_RUN)) {
        ...
        mythread_sync(coder->thr->mutex) {
            if (coder->thr->state == THR_IDLE) {
                ...
                if (finish) coder->thr->state = THR_FINISH;
...
static lzma_ret stream_encode_mt(void *coder_ptr,
    ...
    switch (coder->sequence) {
        case SEQ_STREAM_HEADER:
            ...
            if (coder->header_pos < sizeof(coder->header)) return LZMA_OK;
            ...
            while (true) {
                ...
                if (*in_pos == in_size) {
                    ...
                    if (lzma_outq_is_empty(&coder->outq)) {
                        ...
                        // completed.
                        if (action == LZMA_FULL_FLUSH) return LZMA_STREAM_END;
                    }
                }
                ...
                // return code.
                if (*out_pos == out_size) return LZMA_OK;
...
static lzma_ret get_options(const lzma_mt *options, lzma_options_easy *opt_easy,
    ...
    if (options == NULL) return LZMA_PROG_ERROR;
    ...
    if (options->block_size > 0) {
        if (options->block_size > BLOCK_SIZE_MAX) return LZMA_OPTIONS_ERROR;

        *block_size = options->block_size;
    } else {
        // Determine the Block size from the filter chain.
        *block_size = lzma_mt_block_size(*filters);
        if (*block_size == 0) return LZMA_OPTIONS_ERROR;
    ...
    *outbuf_size_max = lzma_block_buffer_bound64(*block_size);
    if (*outbuf_size_max == 0) return LZMA_MEM_ERROR;
...
static lzma_ret stream_encoder_mt_init(lzma_next_coder *next,
    ...
    if (block_size > SIZE_MAX) return LZMA_MEM_ERROR;
#endif
    ...
    if (!lzma_check_is_supported(options->check)) return LZMA_UNSUPPORTED_CHECK;

    // Allocate and initialize the base structure if needed.
    lzma_stream_coder *coder = next->coder;
    if (coder == NULL) {
        coder = lzma_alloc(sizeof(lzma_stream_coder), allocator);
        if (coder == NULL) return LZMA_MEM_ERROR;
    ...
    if (coder->threads_max != options->threads) {
        ...
        if (coder->threads == NULL) return LZMA_MEM_ERROR;
    ...
    if (coder->index == NULL) return LZMA_MEM_ERROR;
...
extern LZMA_API(uint64_t)
    ...
    if (filters_memusage == UINT64_MAX) return UINT64_MAX;
    ...
    if (outq_memusage == UINT64_MAX) return UINT64_MAX;

    // Sum them with overflow checking.
    uint64_t total_memusage = LZMA_MEMUSAGE_BASE + sizeof(lzma_stream_coder) +
                              options->threads * sizeof(worker_thread);

    if (UINT64_MAX - total_memusage < inbuf_memusage) return UINT64_MAX;

    total_memusage += inbuf_memusage;

    if (UINT64_MAX - total_memusage < filters_memusage) return UINT64_MAX;
