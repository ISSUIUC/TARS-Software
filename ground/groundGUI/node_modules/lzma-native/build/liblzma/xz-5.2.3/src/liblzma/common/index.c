...
extern void lzma_index_prealloc(lzma_index *i, lzma_vli records) {
    if (records > PREALLOC_MAX) records = PREALLOC_MAX;
...
static lzma_vli index_file_size(lzma_vli compressed_base, lzma_vli unpadded_sum,
    ...
    if (file_size > LZMA_VLI_MAX) return LZMA_VLI_UNKNOWN;
...
extern LZMA_API(lzma_ret)
    ...
    if (i == NULL || stream_flags == NULL) return LZMA_PROG_ERROR;
...
extern LZMA_API(lzma_ret)
    ...
    if (g != NULL && g->last + 1 < g->allocated) {
        ...
        g = lzma_alloc(sizeof(index_group) + i->prealloc * sizeof(index_record),
                       allocator);
        if (g == NULL) return LZMA_MEM_ERROR;
...
static void index_cat_helper(const index_cat_info *info, index_stream *this) {
    ...
    if (left != NULL) index_cat_helper(info, left);
    ...
    if (right != NULL) index_cat_helper(info, right);
    ...
    // to be done before modifying dest or src.
    {
        ...
        if (g != NULL && g->last + 1 < g->allocated) {
            ...
            index_group *newg = lzma_alloc(
                ...
                allocator);
            if (newg == NULL) return LZMA_MEM_ERROR;
...
static index_stream *index_dup_stream(const index_stream *src,
    ...
    if (src->record_count > PREALLOC_MAX) return NULL;
    ...
    if (src->groups.leftmost == NULL) return dest;
...
extern LZMA_API(lzma_bool)
    ...
    if ((unsigned int)(mode) > LZMA_INDEX_ITER_NONEMPTY_BLOCK) return true;
...
again:
    if (stream == NULL) {
        ...
        if (group != NULL) group = index_tree_next(&group->node);
...
extern LZMA_API(lzma_bool)
    ...
    if (i->uncompressed_size <= target) return true;
