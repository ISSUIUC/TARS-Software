...
static void io_sandbox_enter(int src_fd) {
    ...
    if (cap_rights_limit(
            src_fd, cap_rights_init(&rights, CAP_EVENT, CAP_FCNTL, CAP_LOOKUP,
                                    CAP_READ, CAP_SEEK)))
        goto error;
...
static io_wait_ret io_wait(file_pair *pair, int timeout, bool is_reading) {
    ...
    while (true) {
        ...
        if (user_abort) return IO_WAIT_ERROR;
        ...
        if (pfd[0].revents != 0) return IO_WAIT_MORE;
    }
}
#endif
...
static bool io_open_src_real(file_pair *pair) {
    ...
    if (pair->src_fd == -1) {
        ...
        if (errno == EMLINK) was_symlink = true;

#elif defined(__digital__) && defined(__unix__)
        if (errno == ENOTSUP) was_symlink = true;

#elif defined(__NetBSD__)
        if (errno == EFTYPE) was_symlink = true;
    ...
    // source file.
#ifdef __DJGPP__
    if (stat(pair->src_name, &pair->src_st)) goto error_msg;
#else
    if (fstat(pair->src_fd, &pair->src_st)) goto error_msg;
#endif
    ...
    if (!S_ISREG(pair->src_st.st_mode)) {
        ...
        if (ret != IO_WAIT_MORE) goto error;
    }
#endif
...
extern file_pair *io_open_src(const char *src_name) {
    if (is_empty_filename(src_name)) return NULL;
...
#ifdef ENABLE_SANDBOX
    if (!error) io_sandbox_enter(pair.src_fd);
#endif
...
static bool io_open_dest_real(file_pair *pair) {
    if (opt_stdout || pair->src_fd == STDIN_FILENO) {
        ...
        if (pair->dest_name == NULL) return true;
    ...
    if (fstat(pair->dest_fd, &pair->dest_st)) {
        ...
        if (pair->dest_fd == STDOUT_FILENO) {
            if (!S_ISREG(pair->dest_st.st_mode)) return false;
            ...
            if (stdout_flags & O_APPEND) {
                ...
                // to imitate O_APPEND.
                if (lseek(STDOUT_FILENO, 0, SEEK_END) == -1) return false;
                ...
                if (restore_stdout_flags) flags |= O_NONBLOCK;
                ...
                if (fcntl(STDOUT_FILENO, F_SETFL, flags) == -1) return false;
...
static bool io_close_dest(file_pair *pair, bool success) {
    ...
    if (pair->dest_fd == -1 || pair->dest_fd == STDOUT_FILENO) return false;
    ...
    // of the junk file.
    if (!success) io_unlink(pair->dest_name, &pair->dest_st);
...
extern void io_close(file_pair *pair, bool success) {
    ...
    if (success && pair->dest_try_sparse && pair->dest_pending_sparse > 0) {
        ...
        if (lseek(pair->dest_fd, pair->dest_pending_sparse - 1, SEEK_CUR) ==
            -1) {
            ...
            if (io_write_buf(pair, zero, 1)) success = false;
        }
    }
    ...
    // the source file!
    if (io_close_dest(pair, success)) success = false;
...
extern bool io_pread(file_pair *pair, io_buf *buf, size_t size, off_t pos) {
    ...
    if (amount == SIZE_MAX) return true;
...
#ifndef TUKLIB_DOSLIKE
            if (IS_EAGAIN_OR_EWOULDBLOCK(errno)) {
                if (io_wait(pair, -1, false) == IO_WAIT_MORE) continue;
...
extern bool io_write(file_pair *pair, const io_buf *buf, size_t size) {
    ...
    if (pair->dest_try_sparse) {
        ...
        if (pair->dest_pending_sparse > 0) {
            if (lseek(pair->dest_fd, pair->dest_pending_sparse, SEEK_CUR) ==
                -1) {
