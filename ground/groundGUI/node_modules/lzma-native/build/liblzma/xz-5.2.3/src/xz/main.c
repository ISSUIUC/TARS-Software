...
extern void set_exit_status(enum exit_status_type new_status) {
    ...
    if (exit_status != E_ERROR) exit_status = new_status;
...
static const char *read_name(const args_info *args) {
    ...
    if (name == NULL) name = xmalloc(size);
...
int main(int argc, char **argv) {
    ...
    if (opt_mode != MODE_LIST) signals_init();
...
#ifdef HAVE_DECODERS
    if (opt_mode == MODE_LIST) run = &list_file;
#endif
    ...
    if (args.files_name != NULL) {
        ...
        if (args.files_name != stdin_filename) (void)fclose(args.files_file);
    }
    ...
    // was specified.
    if (es == E_WARNING && no_warn) es = E_SUCCESS;
