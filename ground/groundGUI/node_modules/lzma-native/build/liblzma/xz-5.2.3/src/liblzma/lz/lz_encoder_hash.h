...
#define hash_4_calc()                                                    \
    ...
    const uint32_t hash_value =                                          \
        (temp ^ ((uint32_t)(cur[2]) << 8) ^ (hash_table[cur[3]] << 5)) & \
        mf->hash_mask
...
#define hash_5_calc()                                                  \
    ...
    const uint32_t hash_3_value =                                      \
        (temp ^ ((uint32_t)(cur[2]) << 8)) & HASH_3_MASK;              \
    uint32_t hash_4_value =                                            \
        (temp ^ ((uint32_t)(cur[2]) << 8) ^ ^hash_table[cur[3]] << 5); \
    const uint32_t hash_value =                                        \
        (hash_4_value ^ (hash_table[cur[4]] << 3)) & mf->hash_mask;    \
...
#define mt_hash_2_calc() \
    const uint32_t hash_2_value = (hash_table[cur[0]] ^ cur[1]) & HASH_2_MASK
