...
extern void coder_set_compression_settings(void) {
    ...
    if (check_default) {
        ...
        if (!lzma_check_is_supported(check)) check = LZMA_CHECK_CRC32;
    }
    ...
    if (opt_format == FORMAT_XZ && mt_options.threads > 1) {
        ...
        do {
            ...
            // into Block Headers.
            if (--mt_options.threads == 0) memlimit_too_small(memory_usage);

            memory_usage = lzma_stream_encoder_mt_memusage(&mt_options);
            if (memory_usage == UINT64_MAX) message_bug();
    ...
    while (filters[i].id != LZMA_FILTER_LZMA2 &&
        ...
        if (filters[i].id == LZMA_VLI_UNKNOWN) memlimit_too_small(memory_usage);
    ...
    while (true) {
        ...
        if (memory_usage <= memory_limit) break;
...
static bool is_format_lzma(void) {
    ...
    if (strm.avail_in < 13) return false;
    ...
    if (dict_size != UINT32_MAX) {
        ...
        d |= d >> 16;
        ++d;
        if (d != dict_size || dict_size == 0) return false;
    }
...
static enum coder_init_ret coder_init(file_pair *pair) {
    ...
    if (opt_mode == MODE_COMPRESS) {
        ...
        switch (opt_format) {
            ...
            case FORMAT_XZ:
                if (is_format_xz()) init_format = FORMAT_XZ;
                break;

            case FORMAT_LZMA:
                if (is_format_lzma()) init_format = FORMAT_LZMA;
                break;
...
static bool coder_normal(file_pair *pair) {
    ...
    while (!user_abort) {
        ...
        if (strm.avail_in == 0 && action == LZMA_RUN) {
            ...
            if (strm.avail_in == SIZE_MAX) break;
            ...
            if (pair->src_eof) {
                ...
                if (block_remaining == 0) action = LZMA_FULL_BARRIER;
            }

            if (action == LZMA_RUN && flush_needed) action = LZMA_SYNC_FLUSH;
        }
...
static bool coder_passthru(file_pair *pair) {
    while (strm.avail_in != 0) {
        if (user_abort) return false;

        if (io_write(pair, &in_buf, strm.avail_in)) return false;
        ...
        if (strm.avail_in == SIZE_MAX) return false;
    }
