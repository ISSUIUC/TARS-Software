...
int main(void) {
    ...
    if (lzma_lzma_preset(&opt_lzma, 1)) return 1;

    lzma_filter filters[] = {{.id = LZMA_FILTER_LZMA2, .options = &opt_lzma},
                             {.id = LZMA_VLI_UNKNOWN}};
    ...
    if (lzma_code(&strm, LZMA_FINISH) != LZMA_STREAM_END) return 1;
    ...
    if (lzma_index_encoder(&strm, idx) != LZMA_OK) return 1;

    if (lzma_code(&strm, LZMA_RUN) != LZMA_STREAM_END) return 1;
    ...
    if (lzma_stream_header_encode(&sf, out) != LZMA_OK) return 1;

    if (lzma_stream_footer_encode(&sf, out + out_size) != LZMA_OK) return 1;
