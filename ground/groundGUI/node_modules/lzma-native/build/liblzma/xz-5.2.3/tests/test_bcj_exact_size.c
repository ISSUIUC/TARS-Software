...
static void compress(void) {
    ...
    expect(lzma_stream_buffer_encode(filters, LZMA_CHECK_CRC32, NULL, in,
                                     sizeof(in), compressed, &compressed_size,
                                     sizeof(compressed)) == LZMA_OK);
}
...
static void decompress(void) {
    ...
    while (true) {
        if (strm.total_in < compressed_size) strm.avail_in = 1;
        ...
        if (strm.total_out < sizeof(in)) strm.avail_out = 1;
    }
}
...
static void decompress_empty(void) {
    ...
    static const uint8_t empty_bcj_lzma2[] = {
        ...
        0x36, 0x02, 0x01, 0x05, 0x00, 0x21, 0x01, 0x00, 0x00, 0x7F, 0xE0,
        0xF1, 0xC8, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x01, 0x11, 0x00, 0x3B, 0x96, 0x5F, 0x73, 0x90, 0x42, 0x99, 0x0D,
    ...
    size_t in_pos = 0;
    size_t out_pos = 0;
    expect(lzma_stream_buffer_decode(&memlimit, 0, NULL, empty_bcj_lzma2,
                                     &in_pos, sizeof(empty_bcj_lzma2), out,
                                     &out_pos, 0) == LZMA_OK);
