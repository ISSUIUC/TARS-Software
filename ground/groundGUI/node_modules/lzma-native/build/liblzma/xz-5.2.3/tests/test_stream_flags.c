...
static bool validate(void) {
    ...
    return lzma_stream_flags_compare(&known_flags, &decoded_flags) != LZMA_OK;
}
...
static bool test_header_decoder(lzma_ret expected_ret) {
    ...
    if (expected_ret != LZMA_OK) return false;
...
static void test_encode_invalid(void) {
    ...
    expect(lzma_stream_header_encode(&known_flags, buffer) == LZMA_PROG_ERROR);

    expect(lzma_stream_footer_encode(&known_flags, buffer) == LZMA_PROG_ERROR);
...
int main(void) {
    // Valid headers
    known_flags.backward_size = 1024;
    for (lzma_check check = LZMA_CHECK_NONE; check <= LZMA_CHECK_ID_MAX;
         ++check) {
        test_header();
        test_footer();
    }
