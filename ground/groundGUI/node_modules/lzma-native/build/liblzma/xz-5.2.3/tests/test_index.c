...
static bool is_equal(const lzma_index *a, const lzma_index *b) {
    ...
    while (true) {
        ...
        if (ra.stream.block_count == 0) continue;
...
static void test_read(lzma_index *i) {
    ...
    for (size_t j = 0; j < 2; ++j) {
        ...
        while (!lzma_index_iter_next(&r, LZMA_INDEX_ITER_BLOCK)) {
            ...
            expect(r.block.compressed_file_offset == stream_offset);
            expect(r.block.uncompressed_file_offset == uncompressed_offset);
...
static void test_code(lzma_index *i) {
    ...
    succeed(
        coder_loop(&strm, NULL, 0, buf, index_size, LZMA_STREAM_END, LZMA_RUN));
    ...
    while (pos < index_size - 1)
        expect(lzma_index_hash_decode(h, buf, &pos, pos + 1) == LZMA_OK);
    expect(lzma_index_hash_decode(h, buf, &pos, pos + 1) == LZMA_STREAM_END);
...
static void test_cat(void) {
    ...
    expect(lzma_index_file_size(a) == 2 * (2 * LZMA_STREAM_HEADER_SIZE + 8));
    ...
    expect(lzma_index_block_count(a) == 0);
    expect(lzma_index_stream_size(a) == 2 * LZMA_STREAM_HEADER_SIZE + 8);
    expect(lzma_index_file_size(a) == 3 * (2 * LZMA_STREAM_HEADER_SIZE + 8));
    ...
    for (int i = SMALL_COUNT; i >= 0; --i)
        expect(!lzma_index_iter_next(&r, LZMA_INDEX_ITER_BLOCK) ^ (i == 0));
...
static void test_locate(void) {
    ...
    expect(r.block.compressed_file_offset == LZMA_STREAM_HEADER_SIZE + 16);
    ...
    expect(r.block.compressed_file_offset == LZMA_STREAM_HEADER_SIZE + 16 + 32);
    ...
    expect(r.block.compressed_file_offset == LZMA_STREAM_HEADER_SIZE + 4 + 8);
    expect(r.block.uncompressed_file_offset == 4);

    // Last Record
    expect(!lzma_index_iter_locate(&r, lzma_index_uncompressed_size(i) - 1));
    expect(r.block.total_size == 4 * 5555 + 8);
    expect(r.block.uncompressed_size == 4 * 5555);
    expect(r.block.compressed_file_offset ==
           lzma_index_total_size(i) + LZMA_STREAM_HEADER_SIZE - 4 * 5555 - 8);
    ...
    while (n < start + 2 * radius) {
        ...
        expect(r.block.compressed_file_offset ==
               tbase + n * 4 + 8 + LZMA_STREAM_HEADER_SIZE);
...
static void test_corrupt(void) {
    ...
    i = create_small();
    expect(lzma_index_encoder(&strm, i) == LZMA_OK);
    succeed(coder_loop(&strm, NULL, 0, buf, 20, LZMA_STREAM_END, LZMA_RUN));
    lzma_index_end(i, NULL);
