...
static bool encode(uint32_t known_size) {
    ...
    uint32_t tmp;
    if (lzma_filter_flags_size(&tmp, &known_flags) != LZMA_OK) return true;

    if (tmp != known_size) return true;

    size_t out_pos = 0;
    if (lzma_filter_flags_encode(&known_flags, buffer, &out_pos, known_size) !=
        LZMA_OK)
        return true;

    if (out_pos != known_size) return true;
...
static bool decode_ret(uint32_t known_size, lzma_ret expected_ret) {
    ...
    size_t pos = 0;
    if (lzma_filter_flags_decode(&decoded_flags, NULL, buffer, &pos,
                                 known_size) != expected_ret ||
        pos != known_size)
        return true;
...
static bool decode(uint32_t known_size) {
    if (decode_ret(known_size, LZMA_OK)) return true;

    if (known_flags.id != decoded_flags.id) return true;
...
static void test_delta(void) {
    ...
    // Test 2
    lzma_options_delta options = {.type = LZMA_DELTA_TYPE_BYTE, .dist = 0};
    known_flags.options = &options;
    expect(encode(99));
