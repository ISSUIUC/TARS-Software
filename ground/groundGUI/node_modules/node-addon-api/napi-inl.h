...
static inline napi_status AttachData(napi_env env, napi_value obj,
                                     FreeType* data,
    ...
    static inline void Wrapper(napi_env env, void* data,
        ...
        WrapVoidCallback([&] {
            FinalizeData* finalizeData =
                static_cast<FinalizeData*>(finalizeHint);
...
inline void Env::SetInstanceData(DataType* data, HintType* hint) {
    napi_status status = napi_set_instance_data(
        ...
        [](napi_env env, void* data, void* hint) {
            fini(env, static_cast<DataType*>(data),
                 static_cast<HintType*>(hint));
        },
        hint);
    NAPI_THROW_IF_FAILED_VOID(_env, status);
}
...
inline size_t BigInt::WordCount() const {
    size_t word_count;
    napi_status status = napi_get_value_bigint_words(_env, _value, nullptr,
                                                     &word_count, nullptr);
    NAPI_THROW_IF_FAILED(_env, status, 0);
    return word_count;
}
...
inline std::string String::Utf8Value() const {
    ...
    value.resize(length);
    status = napi_get_value_string_utf8(_env, _value, &value[0],
...
inline Value Object::operator[](uint32_t index) const { return Get(index); }
...
inline bool Object::Has(const char* utf8name) const {
    bool result;
    napi_status status =
        napi_has_named_property(_env, _value, utf8name, &result);
    NAPI_THROW_IF_FAILED(_env, status, false);
    return result;
}
...
inline Value Object::Get(const char* utf8name) const {
    napi_value result;
    napi_status status =
        napi_get_named_property(_env, _value, utf8name, &result);
    NAPI_THROW_IF_FAILED(_env, status, Value());
    return Value(_env, result);
}
...
inline bool Object::Set(const char* utf8name, const ValueType& value) {
    napi_status status = napi_set_named_property(_env, _value, utf8name,
                                                 Value::From(_env, value));
    NAPI_THROW_IF_FAILED(_env, status, false);
    return true;
}
...
inline void Object::AddFinalizer(Finalizer finalizeCallback, T* data) {
    ...
    napi_status status = details::AttachData(
        _env, *this, data, details::FinalizeData<T, Finalizer>::Wrapper,
        finalizeData);
    if (status != napi_ok) {
        delete finalizeData;
        NAPI_THROW_IF_FAILED_VOID(_env, status);
...
inline ArrayBuffer ArrayBuffer::New(napi_env env, size_t byteLength) {
    ...
    void* data;
    napi_status status =
        napi_create_arraybuffer(env, byteLength, &data, &value);
...
inline void* ArrayBuffer::Data() {
    void* data;
    napi_status status =
        napi_get_arraybuffer_info(_env, _value, &data, nullptr);
    NAPI_THROW_IF_FAILED(_env, status, nullptr);
    return data;
}
...
inline Napi::ArrayBuffer DataView::ArrayBuffer() const {
    ...
    napi_status status = napi_get_dataview_info(
        _env, _value /* dataView */, nullptr /* byteLength */,
        nullptr /* data */, &arrayBuffer /* arrayBuffer */,
        nullptr /* byteOffset */);
...
inline size_t DataView::ByteOffset() const {
    ...
    napi_status status = napi_get_dataview_info(
        _env, _value /* dataView */, nullptr /* byteLength */,
        nullptr /* data */, nullptr /* arrayBuffer */,
...
inline T DataView::ReadData(size_t byteOffset) const {
    if (byteOffset + sizeof(T) > _length ||
        ...
        NAPI_THROW(RangeError::New(
                       _env, "Offset is outside the bounds of the DataView"),
                   0);
    }
...
inline void DataView::WriteData(size_t byteOffset, T value) const {
    if (byteOffset + sizeof(T) > _length ||
        byteOffset + sizeof(T) < byteOffset) {  // overflow
        NAPI_THROW_VOID(RangeError::New(
            _env, "Offset is outside the bounds of the DataView"));
    }
...
inline TypedArrayOf<T>::TypedArrayOf(napi_env env, napi_value value)
    ...
    if (value != nullptr) {
        status = napi_get_typedarray_info(_env, _value, &_type, &_length,
...
static inline napi_status CreateFunction(napi_env env, const char* utf8name,
...
inline Value Function::Call(napi_value recv, size_t argc,
...
inline Value Function::MakeCallback(napi_value recv, size_t argc,
...
inline Reference<T> Reference<T>::New(const T& value,
    ...
    napi_ref ref;
    napi_status status =
        napi_create_reference(env, value, initialRefcount, &ref);
...
inline Reference<T>::Reference(const Reference<T>& other)
    ...
    if (value != nullptr) {
        ...
        // and always creates a strong reference to the given value even if the
        // incoming reference is weak.
        napi_status status = napi_create_reference(_env, value, 1, &_ref);
...
inline void Reference<T>::Reset(const T& value, uint32_t refcount) {
    ...
    if (val != nullptr) {
        napi_status status =
            napi_create_reference(_env, value, refcount, &_ref);
        NAPI_THROW_IF_FAILED_VOID(_env, status);
    }
}
...
inline Napi::Value FunctionReference::Call(napi_value recv, size_t argc,
                                           const napi_value* args) const {
    EscapableHandleScope scope(_env);
...
inline CallbackInfo::CallbackInfo(napi_env env, napi_callback_info info)
    ...
    if (_argc > _staticArgCount) {
        // Use either a fixed-size array (on the stack) or a
        // dynamically-allocated array (on the heap) depending on the number of
        // args.
...
inline PropertyDescriptor PropertyDescriptor::Accessor(
    ...
    return PropertyDescriptor({utf8name, nullptr, nullptr,
                               CbData::GetterWrapper, CbData::SetterWrapper,
                               nullptr, attributes, callbackData});
}
...
inline ClassPropertyDescriptor<T> InstanceWrap<T>::InstanceAccessor(
    ...
    desc.utf8name = utf8name;
    desc.getter =
        getter != nullptr ? T::InstanceGetterCallbackWrapper : nullptr;
    desc.setter =
        setter != nullptr ? T::InstanceSetterCallbackWrapper : nullptr;
    desc.data = callbackData;
    desc.attributes = attributes;
    return desc;
}
...
inline napi_value InstanceWrap<T>::InstanceVoidMethodCallbackWrapper(
    ...
    return details::WrapCallback([&] {
        CallbackInfo callbackInfo(env, info);
        InstanceVoidMethodCallbackData* callbackData =
            reinterpret_cast<InstanceVoidMethodCallbackData*>(
                callbackInfo.Data());
        callbackInfo.SetData(callbackData->data);
...
inline T* ObjectWrap<T>::Unwrap(Object wrapper) {
    T* unwrapped;
    napi_status status = napi_unwrap(wrapper.Env(), wrapper,
...
inline Function ObjectWrap<T>::DefineClass(
    ...
    // We copy the descriptors to a local array because before defining the
    // class we must replace static method property descriptors with value
    // property descriptors such that the value is a function-valued
    // `napi_value` created with `CreateFunction()`.
    //
    // This replacement could be made for instance methods as well, but V8
    // aborts if we do that, because it expects methods defined on the prototype
    // template to have `FunctionTemplate`s.
    ...
    // After defining the class we iterate once more over the property
    // descriptors and attach the data associated with accessors and instance
    // methods to the newly created JavaScript class.
    for (size_t idx = 0; idx < props_count; idx++) {
        ...
        if (prop->getter == T::StaticGetterCallbackWrapper ||
            ...
            // InstanceWrap<T>::AttachPropData is responsible for attaching the
            // data of instance methods and accessors.
...
inline napi_value ObjectWrap<T>::ConstructorCallbackWrapper(
    ...
    if (!isConstructCall) {
        napi_throw_type_error(
            env, nullptr, "Class constructors cannot be invoked without 'new'");
        return nullptr;
    }
    ...
    napi_value wrapper = details::WrapCallback([&] {
        ...
        if (callbackInfo.Env().IsExceptionPending()) {
            // We need to clear the exception so that removing the wrap might
            // work.
...
inline ThreadSafeFunction ThreadSafeFunction::New(
    ...
    napi_status status = napi_create_threadsafe_function(
        ...
        initialThreadCount, finalizeData, wrapper, context, CallJS,
        &tsfn._tsfn);
    if (status != napi_ok) {
        delete finalizeData;
