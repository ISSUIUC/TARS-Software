...
class Value {
   public:
    ...
    ///
    /// An empty value is invalid, and most attempts to perform an operation on
    /// an empty value will result in an exception. Note an empty value is
    ...
    /// `Value` return type may return an empty value to indicate a pending
    /// exception. So when not using C++ exceptions, callers should check
    /// whether the value is empty before attempting to use it.
    bool IsEmpty() const;
    ...
    bool IsTypedArray()
        const;                ///< Tests if a value is a JavaScript typed array.
    ...
    bool IsExternal()
        const;  ///< Tests if a value is a pointer to external data.
...
class Boolean : public Value {
   public:
    ...
    operator bool()
        const;           ///< Converts a Boolean value to a boolean primitive.
...
class Object : public Value {
   public:
    ...
    Value operator[](uint32_t index  ///< Property / element index
    ) const;

    /// Checks whether a property is present.
    bool Has(napi_value key  ///< Property key primitive
    ) const;

    /// Checks whether a property is present.
    bool Has(Value key  ///< Property key
    ) const;
    ...
    /// Checks whether a named property is present.
    bool Has(const std::string& utf8name  ///< UTF-8 encoded property name
    ) const;

    /// Checks whether a own property is present.
    bool HasOwnProperty(napi_value key  ///< Property key primitive
    ) const;

    /// Checks whether a own property is present.
    bool HasOwnProperty(Value key  ///< Property key
    ) const;
    ...
    /// Gets a property.
    Value Get(Value key  ///< Property key
    ) const;
    ...
    /// Gets a named property.
    Value Get(const std::string& utf8name  ///< UTF-8 encoded property name
    ) const;
    ...
    /// Delete property.
    bool Delete(napi_value key  ///< Property key primitive
    );

    /// Delete property.
    bool Delete(Value key  ///< Property key
    );
    ...
    /// Delete property.
    bool Delete(const std::string& utf8name  ///< UTF-8 encoded property name
    );

    /// Checks whether an indexed property is present.
    bool Has(uint32_t index  ///< Property / element index
    ) const;
    ...
    bool Delete(uint32_t index  ///< Property / element index
    );
...
class TypedArray : public Object {
   public:
    ...
    size_t ElementLength()
        const;  ///< Gets the number of elements in the array.
    size_t ByteOffset()
...
class TypedArrayOf : public TypedArray {
   public:
    /// Creates a new TypedArray instance over a new automatically-allocated
    /// array buffer.
    ///
    ...
    static TypedArrayOf New(
        ...
        size_t elementLength,  ///< Length of the created array, as a number of
                               ///< elements
        Napi::ArrayBuffer
            arrayBuffer,      ///< Backing array buffer instance to use
...
class Function : public Object {
   public:
    ...
    static Function New(napi_env env, Callable cb,
                        const char* utf8name = nullptr, void* data = nullptr);
    /// Callable must implement operator() accepting a const CallbackInfo&
    /// and return either void or Value.
    template <typename Callable>
    ...
    Value MakeCallback(napi_value recv, const std::vector<napi_value>& args,
                       napi_async_context context = nullptr) const;
    Value MakeCallback(napi_value recv, size_t argc, const napi_value* args,
                       napi_async_context context = nullptr) const;
...
class Reference {
   public:
    ...
    // within a HandleScope so that the value handle gets cleaned up
    // efficiently.
    T Value() const;
...
class FunctionReference : public Reference<Function> {
   public:
    ...
    Napi::Value Call(napi_value recv,
                     const std::initializer_list<napi_value>& args) const;
    Napi::Value Call(napi_value recv,
                     const std::vector<napi_value>& args) const;
    Napi::Value Call(napi_value recv, size_t argc,
    ...
    Napi::Value MakeCallback(napi_value recv,
                             ...
                             napi_async_context context = nullptr) const;
    Napi::Value MakeCallback(napi_value recv, size_t argc,
...
class InstanceWrap {
   public:
    ...
    static PropertyDescriptor InstanceAccessor(
        ...
        void* data = nullptr);
    static PropertyDescriptor InstanceAccessor(
        Symbol name, InstanceGetterCallback getter,
        InstanceSetterCallback setter,
        napi_property_attributes attributes = napi_default,
        void* data = nullptr);
   ...
   private:
    ...
    static napi_value InstanceVoidMethodCallbackWrapper(
...
class TypedThreadSafeFunction {
   public:
    ...
    template <typename CallbackType, typename ResourceString,
...
class AsyncProgressWorkerBase : public AsyncWorker {
   public:
    ...
    class ThreadSafeData {
       public:
        ThreadSafeData(AsyncProgressWorkerBase* asyncprogressworker,
                       DataType* data)
...
class AsyncProgressWorker : public AsyncProgressWorkerBase<void> {
   public:
    ...
    class ExecutionProgress {
       ...
       private:
        explicit ExecutionProgress(AsyncProgressWorker* worker)
            : _worker(worker) {}
