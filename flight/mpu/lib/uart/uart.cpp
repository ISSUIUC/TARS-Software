...
int UART::uart_init(char* port) {
    ...
    tty.c_cc[VTIME] = 0;  // Wait indefinitely until the minimum number of bytes
                          // have been read.
    tty.c_cc[VMIN] =
        71;  // Only return when this minimum number of bytes are received.

    // Set in/out baud rate to be 9600. NEEDS SAME BAUD RATE AS TEENSY
    // Remember we will need to test for the optimal speed once the teensy's are
    // set up.
