...
static THD_FUNCTION(main_consumer_thread, arg) {
    ...
    for(int i = 0; i < 10000; i++){
        int v1,v2,v3,v4,v5;
        while(!prod_1_fifo.pop(&v1)){chThdSleep(1);};
        while(!prod_2_fifo.pop(&v2)){chThdSleep(1);};
        while(!prod_3_fifo.pop(&v3)){chThdSleep(1);};
        while(!prod_4_fifo.pop(&v4)){chThdSleep(1);};
        while(!prod_5_fifo.pop(&v5)){chThdSleep(1);};
...
void test_many_producer_perf(){
    chThdCreateStatic(producer1_WA,
                sizeof(producer1_WA),
                NORMALPRIO,  /* Initial priority.    */
                many_producer_thread,    /* Thread function.     */
                &prod_1_fifo);       /* Thread parameter.    */
    chThdCreateStatic(producer2_WA,
                sizeof(producer2_WA),
                NORMALPRIO,  /* Initial priority.    */
                many_producer_thread,    /* Thread function.     */
                &prod_2_fifo);       /* Thread parameter.    */
    chThdCreateStatic(producer3_WA,
                sizeof(producer3_WA),
                NORMALPRIO,  /* Initial priority.    */
                many_producer_thread,    /* Thread function.     */
                &prod_3_fifo);       /* Thread parameter.    */
    chThdCreateStatic(producer4_WA,
                sizeof(producer4_WA),
                NORMALPRIO,  /* Initial priority.    */
                many_producer_thread,    /* Thread function.     */
                &prod_4_fifo);       /* Thread parameter.    */
    chThdCreateStatic(producer5_WA,
                sizeof(producer5_WA),
                NORMALPRIO,  /* Initial priority.    */
                many_producer_thread,    /* Thread function.     */
                &prod_5_fifo);       /* Thread parameter.    */
    chThdCreateStatic(main_consumer_WA,
                sizeof(main_consumer_WA),
                NORMALPRIO,  /* Initial priority.    */
...
static THD_FUNCTION(typed_main_consumer_thread, arg) {
    ...
    for(int i = 0; i < 10000; i++){
        int v1,v2,v3,v4,v5;
        while(!typed_prod_1_fifo.pop(&v1)){chThdSleep(1);};
        while(!typed_prod_2_fifo.pop(&v2)){chThdSleep(1);};
        while(!typed_prod_3_fifo.pop(&v3)){chThdSleep(1);};
        while(!typed_prod_4_fifo.pop(&v4)){chThdSleep(1);};
        while(!typed_prod_5_fifo.pop(&v5)){chThdSleep(1);};
