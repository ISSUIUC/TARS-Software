...
class HistoryBuffer {
   ...
   public:
    ...
    void push(float val, float timestamp) {
        // sets the value at the end of the queue to be equal to the desired
        // push value
        arr[endIndex] = val;
        timestampArr[endIndex] = timestamp;

        // since our newest value is at the end index, it becomes our start
        // index
        frontIndex = endIndex;

        // sets the new end index to the next oldest value which will be one
        // less than the previous end index if its currently 0, it needs to wrap
        // around to the back
        if (endIndex == 0) {
            endIndex = size - 1;
        } else {
    ...
    float getCurrentSecondDerivativeAverage() {
        // we want to get velocity values from the altitude, which means that we
        // need the change in altitidue over the change in time we can calculate
        // the change in velocity between the current value and the next value
        // and divide it by the time the array will be 50-1 in length
        float firstDiff[(size / 2) - 1];
        for (unsigned i = 0; i < (size / 2) - 1; i++) {
            // we do arr[index] - arr[index+1] because the lower index is going
            // to be the most recent value, which is why it goes first
        ...
        // we want to get acceleration from velocity, so we do the same thing
        // but this time with the velocity array the new array with be length
        // 49-1
    ...
    float getPastSecondDerivativeAverage() {
        // same as current second derivative average, except we use indexes
        // 50-99
        float firstDiff[(size / 2) - 1];
