...
bool RH_NRF24::setNetworkAddress(uint8_t* address, uint8_t len) {
    ...
    spiWriteRegister(RH_NRF24_REG_03_SETUP_AW,
                     len - 2);  // Mapping [3..5] = [1..3]
...
bool RH_NRF24::setRF(DataRate data_rate, TransmitPower power) {
    ...
    // If we were using auto-ack, we would have to set the appropriate timeout
    // in reg 4 here see NRF24::setRF()
    return true;
}
