...
// corresponding to the bit and field names in the nRF24L01 Product
// Specification #define RH_NRF24_REG_00_CONFIG                             0x00
...
/// \brief Send and receive addressed, reliable, acknowledged datagrams by
/// nRF24L01 and compatible transceivers.
///
/// Supported transceivers include:
/// - Nordic nRF24 based 2.4GHz radio modules, such as nRF24L01
/// http://www.nordicsemi.com/eng/Products/2.4GHz-RF/nRF24L01 and other
/// compatible transceivers.
/// - nRF24L01p with PA and LNA modules that produce a higher power output
/// similar to this one:
/// http://www.elecfreaks.com/wiki/index.php?title=2.4G_Wireless_nRF24L01p_with_PA_and_LNA
/// - Sparkfun WRL-00691 module with nRF24L01
/// https://www.sparkfun.com/products/691 or WRL-00705
/// https://www.sparkfun.com/products/705 etc.
...
/// unaddressed, unreliable datagrams of arbitrary length to 28 octets per
/// packet. Use one of the Manager classes to get addressing and acknowledgement
/// reliability, routing, meshes etc.
///
/// The nRF24L01
/// (http://www.sparkfun.com/datasheets/Wireless/Nordic/nRF24L01P_Product_Specification_1_0.pdf)
/// is a low-cost 2.4GHz ISM transceiver module. It supports a number of channel
/// frequencies in the 2.4GHz band and a range of data rates.
///
/// This library provides functions for sending and receiving messages of up to
/// 28 octets on any frequency supported by the nRF24L01, at a selected data
/// rate.
///
/// Several nRF24L01 modules can be connected to an Arduino, permitting the
...
/// acknowledgement and no retransmits. TX_ADDR and RX_ADDR_P0 are set to the
/// network address. If you need the low level auto-acknowledgement feature
/// supported by this chip, you can use our original NRF24 library at
/// http://www.airspayce.com/mikem/arduino/NRF24
///
/// Naturally, for any 2 radios to communicate that must be configured to use
/// the same frequency and data rate, and with identical network addresses.
///
...
/// as specified by the nRF24L01 product specification:
///
/// - 1 octets PREAMBLE
...
///
/// The electrical connection between the nRF24L01 and the Arduino require 3.3V,
/// the 3 x SPI pins (SCK, SDI, SDO), a Chip Enable pin and a Slave Select pin.
/// If you are using the Sparkfun WRL-00691 module, it has a voltage regulator
/// on board and can be should with 5V VCC if possible. The examples below
/// assume the Sparkfun WRL-00691 module
///
/// Connect the nRF24L01 to most Arduino's like this (Caution, Arduino Mega has
/// different pins for SPI, see below). Use these same connections for
/// Teensy 3.1 (use 3.3V not 5V Vcc). \code
...
/// \endcode
///
/// For an Arduino Leonardo (the SPI pins do not come out on the Digital pins as
/// for normal Arduino, but only appear on the ICSP header) \code
...
/// normal Arduino, but only appear on the SPI header). Use the same connections
/// for Yun with 5V or 3.3V. \code
...
///
/// For an Itead Studio IBoard Pro http://imall.iteadstudio.com/iboard-pro.html,
/// connected by hardware SPI to the ITDB02 Parallel LCD Module Interface pins:
/// \code
...
///
/// For an Itead Studio IBoard Pro http://imall.iteadstudio.com/iboard-pro.html,
/// connected by software SPI to the nRF24L01+ Module Interface pins. CAUTION:
/// performance of software SPI is very slow and is not compatible with other
/// modules running hardware SPI. \code
...
/// \endcode
/// See the example program and Makefile in examples/raspi. Requires bcm2835
/// library to be previously installed. \code cd examples/raspi make sudo
/// ./RasPiRH \endcode \code
///
/// You can override the default settings for the CSN and CE pins
/// in the NRF24() constructor if you wish to connect the slave select CSN to
...
/// mode.
///
/// Caution: this module has not been proved to work with Leonardo, at least
/// without level shifters between the nRF24 and the Leonardo. Tests seem to
/// indicate that such level shifters would be required with Leonardo to make it
/// work.
///
/// It is possible to have 2 radios conected to one arduino, provided each radio
/// has its own CSN and CE line (SCK, SDI and SDO are common to both radios)
///
/// \par SPI Interface
///
/// You can interface to nRF24L01 with with hardware or software SPI. Use of
/// software SPI with the RHSoftwareSPI class depends on a fast enough processor
/// and digitalOut() functions to achieve a high enough SPI bus frequency. If
/// you observe reliable behaviour with the default hardware SPI RHHardwareSPI,
/// but unreliable behaviour with Software SPI RHSoftwareSPI, it may be due to
...
///
/// Frequency accuracy may be debatable. For nominal frequency of 2401.000 MHz
/// (ie channel 1), my Yaesu VR-5000 receiver indicated the center frequency for
/// my test radios was 2401.121 MHz. Its not clear to me if the Yaesu is the
/// source of the error, but I tend to believe it, which would make the nRF24l01
/// frequency out by 121kHz.
///
/// The measured power output for a nRF24L01p with PA and LNA set to 0dBm output
/// is about 18dBm.
///
/// \par Radio operating strategy and defaults
///
/// The radio is enabled all the time, and switched between TX and RX modes
/// depending on whether there is any data to send. Sending data sets the radio
/// to TX mode. After data is sent, the radio automatically returns to Standby
/// II mode. Calling waitAvailable() or waitAvailableTimeout() starts the radio
/// in RX mode.
///
/// The radio is configured by default to Channel 2, 2Mbps, 0dBm power, 5 bytes
/// address, payload width 1, CRC enabled 2 byte CRC, No Auto-Ack mode. Enhanced
/// shockburst is used. TX and P0 are set to the Network address. Node addresses
/// and decoding are handled with the RH_NRF24 module.
///
/// \par Memory
///
/// Memory usage of this class is minimal. The compiled client and server
/// sketches are about 6000 bytes on Arduino. The reliable client and server
/// sketches compile to about 8500 bytes on Arduino. RAM requirements are
/// minimal.
///
class RH_NRF24 : public RHNRFSPIDriver {
   public:
    ...
    /// These are designed to agree with the values for RF_PWR in
    ...
    /// have its own chip enable and slave select pin. After constructing, you
    /// must call init() to initialise the interface and the radio module
    /// \param[in] chipEnablePin the Arduino pin to use to enable the chip for
    /// transmit/receive \param[in] slaveSelectPin the Arduino pin number of the
    /// output to use to select the NRF24 before accessing it. Defaults to the
    /// normal SS pin for your Arduino (D10 for Diecimila, Uno etc, D53 for
    ...
    /// - Set the chip enable and chip select pins to output LOW, HIGH
    /// respectively.
    /// - Initialise the SPI output pins
    /// - Initialise the SPI interface library to 8MHz (Hint, if you want to
    /// lower the SPI frequency (perhaps where you have other SPI shields, low
    /// voltages etc), call SPI.setClockDivider() after init()). -Flush the
    ...
    /// Writes a single byte to the NRF24, and at the same time reads the
    /// current STATUS register \param[in] reg Register number, one of
    /// RH_NRF24_REG_* \param[in] val The value to write \return the current
    /// STATUS (read while the command is sent)
    uint8_t spiWriteRegister(uint8_t reg, uint8_t val);

    /// Reads a number of consecutive registers from the NRF24 using burst read
    /// mode \param[in] reg Register number of the first register, one of
    /// RH_NRF24_REG_* \param[in] dest Array to write the register values to.
    /// Must be at least len bytes \param[in] len Number of bytes to read
    ...
    /// RH_NRF24_REG_* \param[in] src Array of new register values to write.
    /// Must be at least len bytes \param[in] len Number of bytes to write
    /// \return the current STATUS (read while the command is sent)
    ...
    /// the NRF24 NRF24_REG_00_CONFIG register when in Idle mode. This allows
    /// you to change some chip configuration for compatibility with libraries
    /// other than this one. You should not normally need to call this. Defaults
    /// to NRF24_EN_CRC| RH_NRF24_CRCO, which is the standard configuration for
    /// this library (2 byte CRC enabled). \param[in] mode The chip
    /// configuration to be used whe in Idle mode. \return true on success
    bool setOpMode(uint8_t mode);

    /// Sets the Network address.
    /// Only nodes with the same network address can communicate with each
    /// other. You can set different network addresses in different sets of
    /// nodes to isolate them from each other. Internally, this sets the nRF24
    /// TX_ADDR and RX_ADDR_P0 to be the given network address. The default
    /// network address is 0xE7E7E7E7E7 \param[in] address The new network
    /// address. Must match the network address of any receiving node(s).
    ...
    /// Sets the data rate and transmitter power to use. Note that the nRF24 and
    /// the RFM73 have different available power levels, and for convenience, 2
    /// different sets of values are available in the RH_NRF24::TransmitPower
    /// enum. The ones with the RFM73 only have meaning on the RFM73 and
    /// compatible devces. The others are for the nRF24. \param [in] data_rate
    /// The data rate to use for all packets transmitted and received. One of
    /// RH_NRF24::DataRate. \param [in] power Transmitter power. One of
    /// RH_NRF24::TransmitPower. \return true on success
    ...
    /// \return true on success (which does not necessarily mean the receiver
    /// got the message, only that the message was successfully transmitted).
    ...
    /// \return true if the chip is in transmit mode and there is a transmission
    /// in progress
    bool isSending();

    /// Prints the value of all chip registers
    /// to the Serial device if RH_HAVE_SERIAL is defined for the current
    /// platform For debugging purposes only. \return true on success
    bool printRegisters();
