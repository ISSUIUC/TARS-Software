...
class RHGenericDriver {
   public:
    ...
    /// from the Driver.
    /// On most drivers, if there is an uncollected received message, and there
    /// is no message currently bing transmitted, this will also put the Driver
    /// into RHModeRx mode until a message is actually received by the
    /// transport, when it will be returned to RHModeIdle. This can be called
    ...
    /// permitted. If the message is too long for the underlying radio
    /// technology, send() will return false and will not send the message.
    /// \param[in] data Array of data to be sent
    /// \param[in] len Number of bytes of data to send (> 0)
    /// specify the maximum time in ms to wait. If 0 (the default) do not wait
    /// for CAD before transmitting. \return true if the message length was
    /// valid and it was correctly queued for transmit. Return false if CAD was
    /// requested and the CAD timeout timed out before clear channel was
    /// detected.
    ...
    /// Starts the receiver and blocks until a received message is available or
    /// a timeout \param[in] timeout Maximum time to wait in milliseconds.
    ...
    /// randomSeed(analogRead(A0)); in your sketch. Permits the implementation
    /// of listen-before-talk mechanism (Collision Avoidance). Calls the
    ...
    /// radio-specific CAD (as returned by isChannelActive()) shows the channel
    /// is clear within the timeout period (or the timeout period is 0), else
    /// returns false.
    virtual bool waitCAD();

    /// Sets the Channel Activity Detection timeout in milliseconds to be used
    /// by waitCAD(). The default is 0, which means do not wait for CAD
    /// detection. CAD detection depends on support for isChannelActive() by
    ...
    /// documentation. This is called automatically by waitCAD(). \return true
    /// if the radio-specific CAD (as returned by override of isChannelActive())
    /// shows the current radio channel as active, else false. If there is no
    /// radio-specific CAD, returns false.
    virtual bool isChannelActive();

    /// Sets the address of this node. Defaults to 0xFF. Subclasses or the user
    /// may want to change this. This will be used to test the adddress in
    /// incoming messages. In non-promiscuous mode, only messages with a TO
    /// header the same as thisAddress or the broadcast addess (0xFF) will be
    /// accepted. In promiscuous mode, all messages will be accepted regardless
    /// of the TO header. In a conventional multinode system, all nodes will
    /// have a unique address (which you could store in EEPROM). You would
    /// normally set the header FROM address to be the same as thisAddress
    ...
    /// Sets and clears bits in the FLAGS header to be sent in all subsequent
    /// messages First it clears he FLAGS according to the clear argument, then
    /// sets the flags according to the set argument. The default for clear
    /// always clears the application specific flags. \param[in] set bitmask of
    /// bits to be set. Flags are cleared with the clear mask before being set.
    /// \param[in] clear bitmask of flags to clear. Defaults to
    ...
    /// when the preamble is received. If you called readRssi() more recently,
    /// it will return that more recent value. \return The most recent RSSI
    /// measurement in dBm.
    int8_t lastRssi();
    ...
    /// mode. If successful, the transport will stay in sleep mode until woken
    /// by changing mode it idle, transmit or receive (eg by calling send(),
    /// recv(), available() etc) \return true if sleep mode is supported by
    /// transport hardware and the RadioHead driver, and if sleep mode
