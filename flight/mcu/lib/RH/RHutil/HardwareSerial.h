...
/// Additional methods not present on Arduino are also provided for waiting for
/// characters.
///
/// The device port is configured for 8 bits, no parity, 1 stop bit and full raw
/// transparency, so it can be used to send and receive any 8 bit character. A
/// limited range of baud rates is supported.
///
/// \par Device Names
///
/// Device naming conventions vary from OS to OS. ON linux, an FTDI serial port
/// may have a name like /dev/ttyUSB0. On OSX, it might be something like
/// /dev/tty.usbserial-A501YSWL \par errors
///
/// A number of these methods print error messages to stderr in the event of an
/// IO error.
class HardwareSerial {
   public:
    ...
    /// IO failures and unsupported baud rates will result in an error message
    /// on stderr. \param[in] baud The desired baud rate. The only rates
    /// supported are: 50, 75, 110, 134, 150 200, 300, 600, 1200, 1800, 2400,
    /// 4800, 9600, 19200, 38400, 57600, 115200, 230400. On some platform such
    /// as Linux you may also use: 460800, 921600.
    void begin(int baud);

    /// Close the port.
    /// If begin() has previously been called successfully, the device port will
    /// be closed. It may be reopened again with another call to begin().
    void end();
    ...
    /// device.
    /// \return 0 if none available else the number of characters available for
    /// immediate reading
    int available();
    ...
    /// \param[in] ch The character to send. Anything in the range 0x00 to 0xff
    /// is permitted \return 1 if successful else 0
    ...
    /// \param[in] timeout The maximum time to wait in milliseconds. 0 means
    /// wait forever. \return true if a message is available as reported by
    /// available()
