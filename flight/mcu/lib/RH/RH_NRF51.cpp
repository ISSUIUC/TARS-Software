...
bool RH_NRF51::init() {
    ...
    setChannel(2);  // The default, in case it was set by another app without
                    // powering down
...
void RH_NRF51::setModeRx() {
    if (_mode != RHModeRx) {
        ...
        // Radio will transition automatically to Disable state when a message
        // is received
...
void RH_NRF51::setModeTx() {
    if (_mode != RHModeTx) {
        ...
        // Sigh: it seems that it takes longer to start the receiver than the
        // transmitter for this type of radio, so if a message is received and
        // an ACK or reply is sent to soon, the original transmitter may not see
        // the reply. So we delay here to make sure the receiver is ready. Yes,
...
void RH_NRF51::setEncryptionKey(uint8_t* key) {
    ...
    if (key) {
        ...
        // Also need to set SCRATCHPTR temp buffer os size 16+MAXPACKETSIZE in
        // RAM
...
bool RH_NRF51::available() {
    if (!_rxBufValid) {
        ...
        setModeIdle();
#if RH_NRF51_HAVE_ENCRYPTION
        // If encryption is enabled, the decrypted message is not available yet,
        // and there seems to be no way to be sure when its ready, but a delay
        // of 2ms is enough
