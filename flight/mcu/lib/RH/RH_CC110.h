...
/// \brief Send and receive addressed, reliable, acknowledged datagrams by Texas
/// Instruments CC110L and compatible transceivers and modules.
///
/// The TI CC110L is a low cost tranceiver chip capable of 300 to 928MHz and
/// with a wide range of modulation types and speeds. The chip is typically
/// provided on a module that also includes the antenna and coupling hardware
...
/// unaddressed, unreliable datagrams of arbitrary length to 59 octets per
...
///
/// Several CC110L modules can be connected to an Arduino, permitting the
...
/// implementor may configure the mnodem characteristics directly by calling
/// setModemRegisters().
///
...
///
/// Modules based on the CC110L may contain a crystal oscillator with one of 2
/// possible frequencies: 26MHz or 27MHz. A number of radio configuration
/// parameters (including carrier frequency and data rates) depend on the
/// crystal oscillator frequency. The chip has no knowledge of the frequency, so
/// it is up to the implementer to tell the driver the oscillator frequency by
/// passing in the appropriate value of is27MHz to the constructor (default
/// 26MHz) or by calling setIs27MHz() before calling init(). Failure to
/// correctly set this flag will cause incorrect frequency and modulation
/// characteristics to be used.
///
/// Caution: it is not easy to determine what the actual crystal frequency is on
/// some modules. For example, the documentation for the Anaren BoosterPack
/// indictes a 26MHz crystal, but measurements on the devices delivered here
...
///
/// Warning: the CC110L is a 3.3V part, and exposing it to 5V on any pin will
/// damage it. Ensure you are using a 3.3V MCU or use level shifters. We tested
/// with Teensy 3.1.
///
/// The electrical connection between a CC110L module and the Arduino or other
/// processor require 3.3V, the 3 x SPI pins (SCK, SDI, SDO), a Chip Select pin
/// and an Interrupt pin. Examples below assume the Anaren BoosterPack. Caution:
/// the pin numbering on the Anaren BoosterPack is a bit counter-intuitive: the
/// direction of number on J1 is the reverse of J2. Check the pin numbers
...
/// \endcode
/// and use the default RH_CC110 constructor. You can use other pins by passing
/// the appropriate arguments to the RH_CC110 constructor, depending on what
/// your MCU supports.
///
...
/// waitPacketSent() is called when it wil transition to IDLE
...
/// default in the TI SmartRF Suite. This software allows you to get the correct
/// register values for diferent modulation schemes. All the modulation schemes
/// prvided in the driver are based on the recommended register values given by
/// SmartRF. Other schemes such a 2-FSK, 4-FSK and OOK are suported by the chip,
/// but canned configurations are not provided with this driver. The implementer
/// may choose to create their own modem configurations and pass them to
/// setModemRegisters().
///
class RH_CC110 : public RHNRFSPIDriver {
   public:
    ...
    /// versions (though we will try to avoid it). All configs use SYNC_MODE =
    /// RH_CC110_SYNC_MODE_16_16 (2 byte sync)
    typedef enum {
        GFSK_Rb1_2Fd5_2 = 0,  ///< GFSK, Data Rate: 1.2kBaud, Dev: 5.2kHz, RX BW
    ...
    /// Do not change one without changing the other. Use the symbolic names,
    ...
    /// CC110L before accessing it. Defaults to the normal SS pin for your
    ...
    /// the CC110L GDO0 interrupt line. Defaults to pin 2. Caution: You must
    /// specify an interrupt capable pin. On many Arduino boards, there are
    /// limitations as to which pins may be used as interrupts. On Leonardo pins
    /// 0, 1, 2 or 3. On Mega2560 pins 2, 3, 18, 19, 20, 21. On Due and Teensy,
    /// any digital pin. On other Arduinos pins 2 or 3. See
    ...
    /// may be used. \param[in] is27MHz Set to true if your CC110 is equipped
    ...
    /// In particular, ensure you have called setIs27MHz(true) if your module
    /// has a 27MHz crystal oscillator. After init(), the following default
    /// characteristics are set: TxPower: TransmitPower5dBm Frequency: 915.0
    /// Modulation: GFSK_Rb1_2Fd5_2 (GFSK, Data Rate: 1.2kBaud, Dev: 5.2kHz, RX
    /// BW 58kHz, optimised for sensitivity) Sync Words: 0xd3, 0x91 \return true
    ...
    /// a message is actually received by the transport, when it will be
    /// returned to RHModeIdle and available() will return true. This can be
    /// called multiple times in a timeout loop \return true if a new, complete,
    /// error-free uncollected message is available to be retreived by recv()
    virtual bool available();

    /// Turns the receiver on if it not already on (after wiaint gor any
    /// currenly transmitting message to complete). If there is a valid message
    /// available, copy it to buf and return true else return false. If a
    /// message is copied, *len is set to the length (Caution, 0 length messages
    /// are permitted). You should be sure to call this function frequently
    /// enough to not miss any messages It is recommended that you call it in
    /// your main loop. \param[in] buf Location to copy the received message
    /// \param[in,out] len Pointer to available space in buf. Set to the actual
    /// number of octets copied. \return true if a valid message was copied to
    /// buf. The message cannot be retreived again.
    ...
    /// If current mode is Sleep, Rx or Tx changes it to Idle. If the
    /// transmitter or receiver is running, disables them.
    void setModeIdle();
    ...
    /// changing mode to idle, transmit or receive (eg by calling send(),
    /// recv(), available() etc) Caution: there is a time penalty as the radio
    /// takes a finite time to wake from sleep mode. Caution: waking up from
    /// sleep loses values from registers 0x29 through 0x2e \return true if
    /// sleep mode was successfully entered.
    virtual bool sleep();

    /// Set the Power Amplifier power setting.
    /// The PaTable settings are based on are based on the suggested optimum
    /// values for multilayer inductors in the 915MHz frequency band. Per table
    /// 5-15. If these values are not suitable, use setPaTable() directly.
    ...
    /// You must indicate to the driver if your CC110L is equipped with a 27MHz
    /// crystal oscillator (26MHz is the default in the constructor). This
    /// should be called before calling init() if you have a 27MHz crystal. It
    /// can be called after calling init() but you must reset the frequency
    /// (with setFrequency()) and modulation (with setModemConfig()) afterwards.
    /// \param[in] is27MHz Pass true if the CC110L has a 27MHz crystal (default
    /// is true).
    void setIs27MHz(bool is27MHz = true);

    /// Sets the transmitter and receiver
    /// centre frequency.
    /// Caution: permissable frequency bands will depend on you country and
    /// area: consult local authorities. \param[in] centre Frequency in MHz.
    /// 300.0 to 928.0 \return true if the selected frquency centre is within
    /// range
    bool setFrequency(float centre);

    /// Sets all the registers required to configure the data modem in the
    /// CC110, including the data rate, bandwidths etc. You cas use this to
    ...
    /// RH_CC110_REG_04_SYNC1 and RH_CC110_REG_05_SYNC0. Caution: SyncWords
    /// should be set to the same value on all nodes in your network. Nodes with
    /// different SyncWords set will never receive each others messages, so
    /// different SyncWords can be used to isolate different networks from each
    /// other. Default is { 0xd3, 0x91 }. \param[in] syncWords Array of sync
    /// words, 2 octets long \param[in] len Number of sync words to set. MUST
    /// be 2.
   ...
   protected:
    ...
    /// On the CC110L, some registers yield different data when read in burst
    /// mode as opposed to single byte mode. \param[in] reg Register number, one
    /// of RH_CC110_REG (burst mode readable) \return The value of the register
    /// after a burst read
    ...
    /// RH_CC110L_REG_* \param[in] src Array of new register values to write.
