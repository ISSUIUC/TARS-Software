...
/// a Teensy 3.1 like this (tested). \code
...
/// intitalisation: \code rf95.setFrequency(915.0); \endcode which adds up to
/// modifying sample sketches something like: \code #include <SPI.h> #include
/// <RH_RF95.h> RH_RF95 rf95(5, 2); // Rocket Scream Mini Ultra Pro with the
/// RFM95W #define Serial SerialUSB
///
/// void setup()
/// {
///   // Ensure serial flash is not interfering with radio communication on SPI
...
class RH_RF95 : public RHSPIDriver {
   public:
    ...
    /// change the RHReliableDatagram timeout for reliable operations.
    typedef enum {
    ...
    /// the canned configurations in ModemConfigChoice suit you. \param[in]
    /// config A ModemConfig structure containing values for the modem
    ...
    /// useRFO false (ie PA_BOOST enabled). \param[in] power Transmitter power
    /// level in dBm. For RFM95/96/97/98 LORA with useRFO false, valid values
    /// are from +5 to +23. For Modtronix inAir4 and inAir9 with useRFO true (ie
    /// RFO pins in use), valid values are from -1 to 14. \param[in] useRFO If
    /// true, enables the use of the RFO transmitter pins instead of the
    /// PA_BOOST pin (false). Choose the correct setting for your module.
