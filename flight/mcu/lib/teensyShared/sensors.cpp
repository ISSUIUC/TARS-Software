 ...
 *              Ayberk Yaraneri
 *
 * @brief       Sensor (Low-G, High-G, GPS, barometer, voltage sensor) function 
 *              implementations
 *
 ...
 * structs.
 * 
 * Update Oct 16tth: This now also contains overloaded functions to get data 
 *                   from HILSIM instead of sensors.
 *
 */
...
/**
 * @brief Construct a new thd function object to handle data collection from the
 * HILSIM low-g emulation.
 *
 * @param arg Contains pointers to various objects needed by the low-g IMU.
 *
 */
void lowGimuTickFunction(HILSIM* hilsim, LSM9DS1* lsm, DataLogBuffer* data_log_buffer,
                         LowGData* lowG_Data) {
    chSysLock();
    hilsim->lsm_readAccel();
    hilsim->lsm_readGyro();
    hilsim->lsm_readMag();
    chSysUnlock();
...
/**
 * @brief Construct a new thd function object to handle data collection from the
 * HILSIM GPS emulation.
 *
 * @param arg Contains pointers to various objects needed by the GPS.
 *
 */
void gpsTickFunction(HILSIM* hilsim, SFE_UBLOX_GNSS* gps, DataLogBuffer* data_log_buffer,
                     GpsData* gps_data) {
    // get read timestamp
    systime_t timeStamp_GPS = chVTGetSystemTime();

    // get the data with a 20 millisecond timeout
    bool has_data = hilsim->gps_getPVT(20);
    ...
    // Log lat, long, alt, posLock
    // all gps input is in 10^-7 degrees
    float latitude = hilsim->gps_getLatitude();
    float longitude = hilsim->gps_getLongitude();
    // adjust to floating point coordinates
    latitude /= 10000000;
    longitude /= 10000000;
    // altitude input is in mm
    float altitude = hilsim->gps_getAltitude();
    // fixtype 3 means that we have a 3d position fix
    uint32_t fix_type = gps->getFixType();
    bool posLock = (fix_type == 3);
...
/**
 * @brief Construct a new thd function object to handle data collection from the
 * HILSIM emulation of the high-g IMU.
 *
 * @param arg Contains pointers to the various objects needed by the high-g IMU.
 *
 */
void highGimuTickFunction(HILSIM* hilsim, QwiicKX134* highG, DataLogBuffer* data_log_buffer,
                          HighGData* highg_data) {
    // Read data from high g IMU
    chSysLock();
    auto data = hilsim->highg_getAccelData();
    chSysUnlock();
...
/**
 * @brief Construct a new thd function object to handle data collection from the
 * HILSIM emulation of the barometer.
 *
 * @param arg Contains pointers to various objects needed by the barometer.
 *
 */
void barometerTickFunction(HILSIM* hilsim, MS5611* barometer, DataLogBuffer* data_log_buffer,
                           BarometerData* barometer_data) {
    // Reads data from the barometer
    barometer->read(12);
