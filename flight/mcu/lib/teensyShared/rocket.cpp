    ...
    while (true) {
        ...
        switch (
            ...
            case STATE_APOGEE:  //*assumes velocity is <= 0
                ...
                } else if (pointer_struct->sensorDataPointer->state_data
                                   .state_vz < main_thresh_max &&
                           pointer_struct->sensorDataPointer->state_data
                                   .state_vz > main_thresh_min) {
                    rocketTimers.main_time = chVTGetSystemTime();
                    pointer_struct->sensorDataPointer->rocketState_data
                        .rocketState = STATE_MAIN_DETECT;
                } else if (pointer_struct->sensorDataPointer->state_data
                               .state_vz > landed_thresh) {
        ...
        chMtxUnlock(
            &pointer_struct->dataloggerTHDVarsPointer.dataMutex_GPS);  //?

        // check that data can be writen to the rocket state buffer
        if (chSemWaitTimeout(
                &pointer_struct->dataloggerTHDVarsPointer.fifoSpace_RS,
