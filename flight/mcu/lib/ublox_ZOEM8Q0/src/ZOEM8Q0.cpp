...
void ZOEM8Q0::endSPI() { SPI.end(); }

bool ZOEM8Q0::update_data() { return process_GPS_NMEA(ZOEM8Q0_CS); }
...
bool ZOEM8Q0::process_GPS_NMEA(uint16_t CS_pin) {
    ...
    for (int counter = 0; counter < MAX_BUFFER_SIZE; ++counter) {
        ...
        if (buffer_idx > 0 && buffer[buffer_idx - 1] == 0xFF &&
        ...
        switch (parser_state) {
            ...
            case GGA_DETECTED:
                ...
                if (buffer[buffer_idx - 2] == 0x0D &&
                    buffer[buffer_idx - 1] == 0x0A) {
...
bool ZOEM8Q0::decode_xxgga_sentence(uint8_t* buffer, uint16_t dataLength) {
    ...
    // position fix is anything other than 0 since we made it though the loop
...
float ZOEM8Q0::get_latitude() { return latitude; }

float ZOEM8Q0::get_longitude() { return longitude; }

float ZOEM8Q0::get_altitude() { return altitude; }

bool ZOEM8Q0::get_position_lock() { return position_lock; }
