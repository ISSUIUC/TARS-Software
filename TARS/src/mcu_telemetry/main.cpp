...
TaskHandle_t telemetry_buffering_task;
TaskHandle_t telemetry_sending_task;

TaskHandle_t gps_task;

// Telemetry tlm;

void telemetry_buffering(void * pvParameters) {
  while(true) {
    tlm.bufferData();
    Serial.println("buffering executed");
...
void telemetry_sending(void * pvParameters) {
  while(true) {
    tlm.transmit();
    Serial.println("sending executed");
...
void gps_runner(void * pvParameters) {
  while(true) {
...
void setup()
{
  ...
  xTaskCreatePinnedToCore(telemetry_buffering, "telemetry buffering", 10000, NULL, 1, &telemetry_buffering_task, 0);
  xTaskCreatePinnedToCore(telemetry_sending, "telemetry sending", 10000, NULL, 1, &telemetry_sending_task, 0);
  xTaskCreatePinnedToCore(gps_runner, "gps", 10000, NULL, 1, &gps_task, 1);
