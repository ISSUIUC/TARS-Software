...
// http://nicerf.com/manage/upfile/indexbanner/635231050196868750.pdf    (API
// description)
...
// Maximum payload length the RF24 can support, limited by our 1 octet message
// length
...
/// RF24 and compatible radio transceiver.
///
/// Works with
...
/// data messages with Hope-RF RF24 and compatible radio modules, such as the
/// RFM24W module.
///
/// The Hope-RF (http://www.hoperf.com) RF24 family is a low-cost ISM
/// transceiver chip. It supports FSK, GFSK, OOK over a wide range of
/// frequencies and programmable data rates. HopeRF also sell these chips on
/// modules which includes a crystal and antenna coupling circuits: RFM24W,
/// RFM26W and RFM27W
///
...
/// to any frequency from 142.0MHz to 1050.0MHz. Caution: most modules only
/// support a more limited range of frequencies due to antenna tuning.
///
...
///
/// RH_RF24 uses interrupts to detect and handle events in the radio chip. The
/// RF24 family has TX and RX FIFOs of 64 bytes, but through the use of
/// interrupt, the RH_RF24 driver can send longer messages by filling or
/// emptying the FIFOs on-the-fly.
///
...
/// \par Packet Format
///
/// All messages sent and received by this RH_RF24 Driver conform to this packet
/// format:
///
/// - 4 octets PREAMBLE (configurable)
/// - 2 octets SYNC 0x2d, 0xd4 (configurable, so you can use this as a network
/// filter)
/// - Field containing 1 octet of message length and 2 octet CRC protecting this
/// field
/// - Field 2 containing at least 4 octets, and 2 octet CRC protecting this
/// field:
...
/// \endcode
/// Caution: tying the radio SDN pin to ground (though it might appear from the
/// data sheets to make sense) does not always produce a reliable radio startup.
/// So this driver controls the SDN pin directly. Note: the GPIO0-TX_ANT and
/// GPIO1-RX_ANT connections are not required for the 11dBm RFM24W, which has no
/// antenna switch.
///
...
///
/// The RH_RF24 module uses a radio configuration header file to configure the
/// basic radio operation frequency and modulation scheme. The radio
/// configuration header file must be generated with the Silicon Labs Wireless
/// Development Suite (WDS) program and #included by RH_RF24.cpp
///
/// The library will work out of the box and without further configuring with
/// these parameters:
/// - Si4464 or equvalent
/// - 30MHz Crytstal
...
///
/// In order to use different frequency bands or modulation schemes, you must
/// generate a new radio configuration header file with WDS, or select one of a
/// small set of prebuilt headers from the RF24configs folder (see README in
/// that folder for details of the filename format).
///
...
/// - On the Frequency and Power tab, Select the Frequency, crystal frequency
/// etc. The PA power level is irrelevant,
...
/// \par RSSI
///
/// The RSSI (Received Signal Strength Indicator) is measured and latched after
/// the message sync bytes are received. The latched RSSI is available from the
/// lastRssi() member functionafter the complete message is received. Although
/// lastRssi() supposedly returns a signed integer, in the case of this radio it
/// actually returns an unsigned 8 bit integer (uint8_t) and you will have to
/// cast the return value to use it: \code uint8_t lastRssi =
/// (uint8_t)rf24.lastRssi(); \endcode The units of RSSI are arbitrary and
/// relative, with larger unsigned numbers indicating a stronger signal. Values
/// up to 255 are seen with radios in close proximity to each other. Lower limit
/// of receivable strength is about 70.
///
...
/// 0x00 will yield no measurable power. For other settings there is a
/// non-linear correlation with actual RF power output (see below) The default
/// is 0x10. Eg: \code driver.setTxPower(0x10); \endcode
///
/// We have made some actual power measurements against
/// programmed power
/// - Anarduino Mini with RFM24-433 and RFM26-433 at Vcc = 3.3V, in CW mode,
/// 434MHz
...
///    0x7f                                      19.2
/// \endcode
/// Caution: the actual radiated power output will depend heavily on the power
/// supply voltage and the antenna.

class RH_RF24 : public RHSPIDriver {
   public:
    /// \brief Defines property values for a set of modem configuration
    /// registers
    ///
    ...
    typedef struct {
        ...
        uint8_t prop_2005;  ///< Value for property
                            ///< RH_RF24_PROPERTY_MODEM_DATA_RATE_0
        uint8_t prop_2006;  ///< Value for property
                            ///< RH_RF24_PROPERTY_MODEM_TX_NCO_MODE_3
        uint8_t prop_2007;  ///< Value for property
                            ///< RH_RF24_PROPERTY_MODEM_TX_NCO_MODE_2
        uint8_t prop_2008;  ///< Value for property
                            ///< RH_RF24_PROPERTY_MODEM_TX_NCO_MODE_1
        uint8_t prop_2009;  ///< Value for property
                            ///< RH_RF24_PROPERTY_MODEM_TX_NCO_MODE_0
        uint8_t prop_200a;  ///< Value for property
                            ///< RH_RF24_PROPERTY_MODEM_FREQ_DEV_2
        uint8_t prop_200b;  ///< Value for property
                            ///< RH_RF24_PROPERTY_MODEM_FREQ_DEV_1
        uint8_t prop_200c;  ///< Value for property
                            ///< RH_RF24_PROPERTY_MODEM_FREQ_DEV_0
        uint8_t prop_2018;  ///< Value for property
                            ///< RH_RF24_PROPERTY_MODEM_TX_RAMP_DELAY
        uint8_t prop_201e;  ///< Value for property
        ...
        uint8_t prop_2026;  ///< Value for property
                            ///< RH_RF24_PROPERTY_MODEM_BCR_NCO_OFFSET_0
        uint8_t prop_2027;  ///< Value for property
        ...
        uint8_t prop_2030;  ///< Value for property
                            ///< RH_RF24_PROPERTY_MODEM_AFC_LIMITER_1
        uint8_t prop_2031;  ///< Value for property
                            ///< RH_RF24_PROPERTY_MODEM_AFC_LIMITER_0
        uint8_t prop_2035;  ///< Value for property
                            ///< RH_RF24_PROPERTY_MODEM_AGC_CONTROL
        uint8_t prop_2038;  ///< Value for property
                            ///< RH_RF24_PROPERTY_MODEM_AGC_WINDOW_SIZE
        uint8_t prop_2039;  ///< Value for property
                            ///< RH_RF24_PROPERTY_MODEM_AGC_RFPD_DECAY
        uint8_t prop_203a;  ///< Value for property
                            ///< RH_RF24_PROPERTY_MODEM_AGC_IFPD_DECAY
        uint8_t prop_203b;  ///< Value for property
    ...
    /// possible that values will be changed in later versions (though we will
    /// try to avoid it). Contributions of new complete and tested ModemConfigs
    /// ready to add to this list will be readily accepted. OBSOLETE: no need
    /// ever to use this now
    typedef enum {
    ...
    /// They deliberately have the same numeric values as the CRC_POLYNOMIAL
    /// field of PKT_CRC_CONFIG
    typedef enum {
        CRC_NONE = 0,
        CRC_ITU_T,
        CRC_IEC_16,
    ...
    /// RF24 before accessing it. Defaults to the normal SS pin for your Arduino
    /// (D10 for Diecimila, Uno etc, D53 for Mega, D10 for Maple) \param[in]
    /// interruptPin The interrupt Pin number that is connected to the RF24 DIO0
    /// interrupt line. Defaults to pin 2. Caution: You must specify an
    /// interrupt capable pin. On many Arduino boards, there are limitations as
    /// to which pins may be used as interrupts. On Leonardo pins 0, 1, 2 or 3.
    /// On Mega2560 pins 2, 3, 18, 19, 20, 21. On Due and Teensy, any digital
    /// pin. On other Arduinos pins 2 or 3. See
    /// http://arduino.cc/en/Reference/attachInterrupt for more details. On
    /// Chipkit Uno32, pins 38, 2, 7, 8, 35. On other boards, any digital pin
    /// may be used. \param [in] sdnPin The pin number connected to SDN on the
    /// radio. Defaults to pin 9.
    ///                     Connecting SDN directly to ground does not aloways
    ...
    /// - Initialise the slave select and shutdown pins and the SPI interface
    /// library
    ...
    /// Sets the chip mode that will be used when the RH_RF24 driver is idle (ie
    /// not transmitting or receiving) You can use this to control the power
    /// level consumed while idle, at the cost of slower transition to tranmit
    /// or receive states \param[in] idleMode The chip state to use when idle.
    /// Sensible choices might be RH_RF24_DEVICE_STATE_SLEEP or
    ...
    /// 119MHz to 960MHz.
    /// Caution: RFM modules are designed with antenna coupling components to
    /// suit a limited band of frequencies (marked underneath the module). It is
    /// possible to set frequencies in other bands, but you may only get little
    /// or no power radiated. Caution, you can only use this function to change
    /// frequency within the frequency band configured by the radio
    /// configuration header file. To use a frequency in a different band, you
    /// must recompile with the appropriate radio configuration header file.
    /// Setting a frequency in anotehr band will have unpredicatble results.
    /// \param[in] centre Frequency in MHz.
    /// \param[in] afcPullInRange Not used
    /// \return true if the selected frequency is within a valid range for the
    /// connected radio and if
    ///         setting the new frequency succeeded.
    bool setFrequency(float centre, float afcPullInRange = 0.05);

    /// OBSOLETE, do not use.
    /// To get different modulation schemes, you must generate a new radio
    /// config file as described in this documentation. Sets all the properties
    /// required to configure the data modem in the RF24, including the data
    /// rate, bandwidths etc. You can use this to configure the modem with
    /// custom configurations if none of the canned configurations in
    ...
    /// OBSOLETE, do not use.
    /// To get different modulation schemes, you must generate a new radio
    /// config file as described in this documentation. Select one of the
    /// predefined modem configurations. If you need a modem configuration not
    /// provided here, use setModemRegisters() with your own ModemConfig. The
    /// default after init() is RH_RF24::GFSK_Rb5Fd10. \param[in] index The
    /// configuration choice. \return true if index is a valid choice.
    ...
    /// value on all nodes in your network. Nodes with different SyncWords set
    /// will never receive each others messages, so different SyncWords can be
    /// used to isolate different networks from each other. Default is { 0x2d,
    /// 0xd4 }. \param[in] syncWords Array of sync words, 1 to 4 octets long.
    /// NULL if no sync words to be used. \param[in] len Number of sync words to
    /// set, 1 to 4. 0 if no sync words to be used.
    ...
    /// The power argument to this function has a non-linear correlation with
    /// the actual RF power output. See the transmitter power table above for
    /// some examples. Also the Si446x Data Sheet section 5.4.2 may be helpful.
    /// Be a good neighbour and set the lowest power level you need.
    /// Caution: legal power limits may apply in certain countries.
    /// After init(), the power will be set to 0x10.
    /// \param[in] power Transmitter power level. For RFM24/Si4460, valid values
    /// are 0x00 to 0x4f. For others, 0x00 to 0x7f
    ...
    /// platform Not all commands have valid replies, therefore they are not all
    /// printed. Caution: the list is very long
    bool printRegisters();

    /// Send a string of command bytes to the chip and get a string of reply
    /// bytes Different RFM24 commands take different numbers of command bytes
    /// and send back different numbers of reply bytes. See the Si446x
    /// documentaiton for more details. Both command bytes and reply bytes are
    /// optional \param[in] cmd The command number. One of RH_RF24_CMD_*
    /// \param[in] write_buf Pointer to write_len bytes of command input bytes
    /// to send. If there are none, set to NULL. \param[in] write_len The number
    /// of bytes to send from write_buf. If there are none, set to 0 \param[out]
    /// read_buf Pointer to read_len bytes of storage where the reply stream
    /// from the comand will be written.
    ...
    /// properties are available. param[in] firstProperty The property number of
    /// the first property to set. The first value in the values array
    ///           will be used to set this property, and any subsequent values
    ///           will be used to set the following properties. One of
    ///           RH_RF24_PROPERTY_*
    ...
    /// command. See the Si446x API Description AN625 for details on what
    /// properties are available. param[in] firstProperty The property number of
    /// the first property to get. The first value in the values array
    ///           will be set with this property, and any subsequent values will
    ///           be set from the following properties. One of
    ///           RH_RF24_PROPERTY_*
    /// param[out] values Array of 0 or more values to receive the firstProperty
    /// and subsequent proerties param[in] count The number of values in the
    /// values array \return true if the command succeeeded.
    ...
    /// Measures and returns the current
    /// voltage applied to a GPIO pin (which has previously been configured as a
    /// voltage input) \param[in] gpio The GPIO pin to read. 0 to 3. \return The
    /// current pin voltage in Volts.
    float get_gpio_voltage(uint8_t gpio);

    /// Read one of the Fast Read Response registers.
    /// The Fast Read Response register must be previously configured with the
    /// matching RH_RF24_PROPERTY_FRR_CTL_?_MODE property to select what chip
    /// property will be available in that register. \param[in] reg The index of
    /// the FRR register to read. 0 means FRR A, 1 means B etc. \return the
    /// value read from the specified Fast Read Response register.
    ...
    /// takes a finte time to wake from sleep mode. \return true if sleep mode
    /// was successfully entered.
    virtual bool sleep();

    /// Return the integer value of the device type
    /// as read from the device in from RH_RF24_CMD_PART_INFO.
    /// One of 0x4460, 0x4461, 0x4462 or 0x4463, depending on the type of device
    /// actually connected. \return The integer device type
    uint16_t deviceType() { return _deviceType; };

   protected:
    /// This is a low level function to handle the interrupts for one instance
    /// of RF24. Called automatically by isr*() Should not need to be called by
    /// user code.
    ...
    /// Copies the next part of the currenrtly received message from the chips
    /// RX FIFO to the receive buffer
    ...
    /// \param[in] commands Array of data containing radio commands in the
    /// format provided by radio_config_Si4460.h \return true if successful
   ...
   private:
    ...
    /// Array of octets of the last received message or the next to transmit
    /// message
