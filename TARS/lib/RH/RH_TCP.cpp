    ...
    if (count < 0) {
        ...
        while (socketBufLen >= 5) {
            ...
            if (len > sizeof(socketBuf) - sizeof(message->length)) {
                // Bogus length
                fprintf(stderr,
                        "RH_TCP::checkForEvents read ridiculous length: %d. "
                        "Corrupt message stream? Aborting\n",
                        len);
                exit(1);
            }
...
bool RH_TCP::send(const uint8_t* data, uint8_t len) {
    if (!waitCAD())
        return false;  // Check channel activity (prob not possible for this
                       // driver?)

    bool ret = sendPacket(data, len);
    delay(10);  // Wait for transmit to succeed. REVISIT: depends on length and
                // speed
    return ret;
}
