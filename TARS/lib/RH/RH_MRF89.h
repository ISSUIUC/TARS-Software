...
/// Microchip MRF89XA and compatible transceivers. and modules.
///
/// The Microchip MRF89XA
/// http://ww1.microchip.com/downloads/en/DeviceDoc/70622C.pdf is a low cost
/// 900MHz bancd transceiver chip. It is commonly used on preassembled modules
/// with supporting circcuits and antennas, such as the MRF89XAM9A
/// http://www.microchip.com/wwwproducts/Devices.aspx?product=MRF89XAM9A This
/// class supports all such modules
///
...
///
/// Several MRF89XA modules can be connected to an Arduino, permitting the
/// construction of translators and frequency changers, etc. Each instance
/// requires 2 chip select pins, and interrupt pin the standard 3 SPI pins.
///
...
///
/// The electrical connection between the MRF89XA and the Arduino require 3.3V,
/// the 3 x SPI pins (SCK, SDI, SDO), a 2 Chip Select pins (/CSCON and /CSDAT)
/// and an interrupt.
///
/// Caution: the MRF89XA is a 3.3V part and is not tolerant of 5V inputs.
/// Connecting MRF89XA directly to a 5V MCU such as most Arduinos will damage
/// the MRF89XA.
///
/// Connect the MRF89XA to most 3.3V Arduinos or Teensy 3.1 like this (use 3.3V
/// not 5V). \code
///                 Teensy       MRF89XAM9A
///               3.3V-----------VIN   (3.3V in)
...
///                 GND----------GND   (ground in)
/// \endcode
/// You can use other pins for /CSDAT, /CSCON, IRQ1 by passing appropriate
...
class RH_MRF89 : public RHNRFSPIDriver {
   public:
    ...
    typedef struct {
        ...
        uint8_t FILCREG;   ///< Value for PASFILV | BUTFILV in
    ...
    /// have its own interrupt and 2 slave select pins. After constructing, you
    ...
    /// csconPin the Arduino pin number connected to the CSCON pin of the
    /// MRF89XA. Defaults to the normal SS pin for your Arduino (D10 for
    /// Diecimila, Uno etc, D53 for Mega, D10 for Maple) \param[in] csdatPin the
    /// Arduino pin number connected to the CSDAT pin of the MRF89XA. Defaults
    /// to 9. \param[in] interruptPin The interrupt Pin number that is connected
    /// to the IRQ1 pin of the MRF89XA. Defaults to pin 2. (IRQ0 pin of the
    ...
    /// RH_MRF89_REG_1A_TXCONREG. Be a good neighbour and set the lowest power
    /// level you need. After init(), the power will be set to
    /// RH_MRF89_TXOPVAL_1DBM (1dBm) The highest power available is
    /// RH_MRF89_TXOPVAL_13DBM (13dBm) Caution: In some countries you may only
    /// select certain higher power levels if you are also using frequency
    /// hopping. Make sure you are aware of the legal limitations and
    /// regulations in your region. Caution: in some countries the maximum
    /// permitted power level may depend on the Bit rate \param[in] power
    /// Transmitter power level, one of RH_MRF89_TXOPVAL*
    ...
    /// returned to RHModeIdle. This can be called multiple times in a timeout
    /// loop \return true if a new, complete, error-free uncollected message is
    ...
    /// Caution not all freqs are supported on all modules: check your module
    /// specifications Caution: not all available and supported frequencies are
    /// legal in every country: check Regulatory Approval eg for MRF89XAM9A (in
    /// 75015B.pdf) Australia 915.0 to 928.0
    ...
    /// RH_MRF89_REG_16_SYNCV31REG et seq. Caution: SyncWords should be set to
    /// the same value on all nodes in your network. Nodes with different
    /// SyncWords set will never receive each others messages, so different
    /// SyncWords can be used to isolate different networks from each other.
    /// Default is { 0x69, 0x81, 0x7e, 0x96 }. Caution, sync words of 2 bytes
    /// and less do not work well with this chip. \param[in] syncWords Array of
    /// sync words, 1 to 4 octets long \param[in] len Number of sync words to
    /// set, 1 to 4.
   ...
   protected:
    ...
    /// \return the current value of RH_MRF89_REG_00_GCONREG (read while the
    /// command is sent)
    ...
    /// \param[in] data Pointer to a buffer containing the len bytes to be
    /// written \param[in] len The number of bytes to write to teh FIFO \return
    /// 0;
