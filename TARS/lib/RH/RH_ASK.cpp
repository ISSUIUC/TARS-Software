...
// insist on using timer 1 Should be moved to header file
...
RH_ASK::RH_ASK(uint16_t speed, uint8_t rxPin, uint8_t txPin, uint8_t pttPin, bool pttInverted)
    : _speed(speed), _rxPin(rxPin), _txPin(txPin), _pttPin(pttPin), _pttInverted(pttInverted), _rxInverted(false) {
...
void RH_ASK::timerSetup() {
    ...
    // REVISIT: does not correctly handle 1MHz clock speeds, only works with
    // 8MHz clocks At 1MHz clock, get 1/8 of the expected baud rate
...
#define RH_ASK_ZERO_TIMER_IRQ TC3_IRQn
    ...
    // Compute the count required to achieve the requested baud (with 8
    // interrupts per bit)
...
bool RH_ASK::send(const uint8_t* data, uint8_t len) {
    ...
    uint8_t count = len + 3 + RH_ASK_HEADER_LEN;  // Added byte count and FCS and headers to
    ...
    if (_rxPllRamp >= RH_ASK_RX_RAMP_LEN) {
        ...
        if (_rxActive) {
            ...
            if (++_rxBitCount >= 12) {
                ...
                // FCS bytes REVISIT: may also include the ACK flag at 0x40
                if (_rxBufLen == 0) {
                    // The first byte is the byte count
                    // Check it for sensibility. It cant be less than 7, since
                    // it includes the byte count itself, the 4 byte header and
                    // the 2 byte FCS
                    _rxCount = this_byte;
