...
// Limited by the single message length octet in the header. Yes, 255 is correct
// even though the FIFO size in the RF22 is only 64 octets. We use interrupts to
// refill the Tx FIFO during transmission and to empty the Rx FIFO during
// reception Can be pre-defined to a smaller size (to save SRAM) prior to
...
// These values we set for FIFO thresholds (4, 55) are actually the same as the
// POR values
...
/// \brief Driver to send and receive unaddressed, unreliable datagrams via an
/// RF22 and compatible radio transceiver.
///
/// Works with RF22, RF23 based radio modules, and compatible chips and modules,
/// including:
/// - RF22 bare module: http://www.sparkfun.com/products/10153
///   (Caution, that is a 3.3V part, and requires a 3.3V CPU such as Teensy etc
///   or level shifters)
...
/// - Silicon Labs Si4430/31/32 based modules. S4432 is equivalent to RF22.
/// Si4431/30 is equivalent to RF23.
///
/// Data based on
/// https://www.sparkfun.com/datasheets/Wireless/General/RFM22B.pdf
///
...
/// unaddressed, unreliable datagrams of arbitrary length to 255 octets per
/// packet.
///
...
///
/// On transmission, the TO and FROM addresses default to 0x00, unless changed
/// by a subclass. On reception the TO addressed is checked against the node
/// address (defaults to 0x00) or the broadcast address (which is 0xff). The ID
/// and FLAGS are set to 0, and not checked by this class. This permits use of
/// the this base RH_RF22 class as an unaddressed, unreliable datagram service
/// without the use of one the RadioHead Manager classes.
///
...
/// data messages with Hope-RF RF22 and RF23 based radio modules, and compatible
/// chips and modules, including the RFM22B transceiver module such as this bare
/// module: http://www.sparkfun.com/products/10153 and this shield:
/// http://www.sparkfun.com/products/11018 and this module:
/// http://www.hoperfusa.com/details.jsp?pid=131 and this integrated board:
/// http://www.anarduino.com/miniwireless and RF23BP modules such as this
/// http://www.anarduino.com/details.jsp?pid=130
///
/// The Hope-RF (http://www.hoperf.com) RFM22B
/// (http://www.hoperf.com/rf_fsk/fsk/RFM22B.htm) is a low-cost ISM transceiver
/// module. It supports FSK, GFSK, OOK over a wide range of frequencies and
/// programmable data rates. Manual can be found at
/// https://www.sparkfun.com/datasheets/Wireless/General/RFM22.PDF
///
/// This library provides functions for sending and receiving messages of up to
/// 255 octets on any frequency supported by the RF22B, in a range of predefined
/// data rates and frequency deviations. Frequency can be set with 312Hz
/// precision to any frequency from 240.0MHz to 960.0MHz.
///
/// Up to 3 RF22B modules can be connected to an Arduino, permitting the
/// construction of translators and frequency changers, etc.
///
/// The following modulation types are suppported with a range of modem
/// configurations for common data rates and frequency deviations:
/// - GFSK Gaussian Frequency Shift Keying
/// - FSK Frequency Shift Keying
/// - OOK On-Off Keying
///
/// Support for other RF22B features such as on-chip temperature measurement,
...
///    - Version Code = 0x06
/// Works on Duo. Works with Sparkfun RFM22 Wireless shields. Works with RFM22
/// modules from http://www.hoperfusa.com/ Works with Arduino 1.0 to at
/// least 1.0.5. Works on Maple, Flymaple, Uno32 (with ChipKIT Core with Arduino
/// IDE).
///
/// \par Packet Format
///
/// All messages sent and received by this Driver must conform to this packet
/// format:
///
...
///
/// For technical reasons, the message format is not protocol compatible with
/// the 'HopeRF Radio Transceiver Message Library for Arduino'
/// http://www.airspayce.com/mikem/arduino/HopeRF from the same author. Nor is
/// it compatible with 'Virtual Wire'
/// http://www.airspayce.com/mikem/arduino/VirtualWire.pdf also from the same
/// author.
///
/// \par Connecting RFM-22 to Arduino
///
/// If you have the Sparkfun RFM22 Shield
/// (https://www.sparkfun.com/products/11018) the connections described below
/// are done for you on the shield, no changes required, just add headers and
/// plug it in to an Arduino (but not and Arduino Mega, see below)
///
...
///
/// Note also that on the RFM22B (but not the RFM23B), it is required to control
/// the TX_ANT and RX_ANT pins of the RFM22 in order to control the antenna
/// connection properly. The RH_RF22 driver is configured by default so that
/// GPIO0 and GPIO1 outputs can control TX_ANT and RX_ANT input pins
/// respectively automatically. On RFM22, you must connect GPIO0 to TX_ANT and
/// GPIO1 to RX_ANT for this automatic antenna switching to occur.  See
/// setGpioReversed() for more details. These connections are not required on
/// RFM23B.
///
/// If you are using the Sparkfun RF22 shield, it will work with any 5V arduino
/// without modification. Connect the RFM-22 module to most Arduino's like this
/// (Caution, Arduino Mega has different pins for SPI, see below). \code
///                 Arduino      RFM-22B
///                 GND----------GND-\ (ground in)
///                              SDN-/ (shutdown in)
///                 3V3----------VCC   (3.3V in)
...
///                           /--GPIO0 (GPIO0 out to control transmitter antenna
///                           TX_ANT)
...
///                           \--RX_ANT (RX antenna control in) RFM22B only
/// \endcode
/// For Chipkit Uno32. Caution: you must also ensure jumper JP4 on the Uno32 is
/// set to RD4 \code
///                 Arduino      RFM-22B
///                 GND----------GND-\ (ground in)
///                              SDN-/ (shutdown in)
...
///                           \--RX_ANT (RX antenna control in) RFM22B only
/// \endcode
/// For an Arduino Due (the SPI pins do not come out on the Digital pins as for
/// normal Arduino, but only appear on the SPI header) \code
///                 Due      RFM-22B
///                 GND----------GND-\ (ground in)
///                              SDN-/ (shutdown in)
///                 5V-----------VCC   (5V in)
...
/// RH_RF22 driver;
/// For connecting an Arduino to an RFM23BP module. Note that the antenna
/// control pins are reversed compared to the RF22. \code
///                 Arduino      RFM-23BP
///                 GND----------GND-\ (ground in)
///                              SDN-/ (shutdown in)
///                 5V-----------VCC   (5V in)
...
///        MOSI pin D11----------SDI   (SPI Data in)
///        MISO pin D12----------SDO   (SPI data out)
///                           /--GPIO0 (GPIO0 out to control receiver antenna
///                           RXON)
///                           \--RXON   (RX antenna control in)
///                           /--GPIO1 (GPIO1 out to control transmitter antenna
///                           TXON)
...
/// in the RH_RF22 constructor if you wish to connect the slave select SS to
/// other than the normal one for your Arduino (D10 for Diecimila, Uno etc and
/// D53 for Mega) or the interrupt request to other than pin D2 (Caution,
/// different processors have different constraints as to the pins available for
/// interrupts).
///
/// If you have an Arduino Zero, you should note that you cannot use Pin 2 for
/// the interrupt line (Pin 2 is for the NMI only), instead you can use any
/// other pin (we use Pin 3) and initialise RH_RF69 like this: \code
/// // Slave Select is pin 10, interrupt is Pin 3
/// RH_RF22 driver(10, 3);
/// \endcode
///
/// It is possible to have 2 radios connected to one Arduino, provided each
/// radio has its own SS and interrupt line (SCK, SDI and SDO are common to both
/// radios)
///
/// Caution: on some Arduinos such as the Mega 2560, if you set the slave select
/// pin to be other than the usual SS pin (D53 on  Mega 2560), you may need to
/// set the usual SS pin to be an output to force the Arduino into SPI master
/// mode.
///
/// Caution: Power supply requirements of the RF22 module may be relevant in
/// some circumstances: RF22 modules are capable of pulling 80mA+ at full power,
/// where Arduino's 3.3V line can give 50mA. You may need to make provision for
/// alternate power supply for the RF22, especially if you wish to use full
/// transmit power, and/or you have other shields demanding power. Inadequate
/// power for the RF22 is reported to cause symptoms such as:
...
/// doesn't happen -shields hang Arduino boards, especially during the flashing
///
/// Caution: some RF22 breakout boards (such as the HAB-RFM22B-BOA
/// HAB-RFM22B-BO) reportedly have the TX_ANT and RX_ANT pre-connected to GPIO0
/// and GPIO1 round the wrong way. You can work with this if you use
/// setGpioReversed().
///
/// Caution: If you are using a bare RF22 module without IO level shifters, you
/// may have difficulty connecting to a 5V arduino. The RF22 module is 3.3V and
/// its IO pins are 3.3V not 5V. Some Arduinos (Diecimila and Uno) seem to work
/// OK with this, and some (Mega) do not always work reliably. Your Mileage May
/// Vary. For best result, use level shifters, or use a RF22 shield or board
/// with level shifters built in, such as the Sparkfun RFM22 shield
/// http://www.sparkfun.com/products/11018. You could also use a 3.3V IO Arduino
/// such as a Pro. It is recognised that it is difficult to connect the Sparkfun
/// RFM22 shield to a Mega, since the SPI pins on the Mega are different to
/// other Arduinos, But it is possible, by bending the SPI pins (D10, D11, D12,
/// D13) on the shield out of the way before plugging it in to the Mega and
/// jumpering the shield pins to the Mega like this: \code
///   RF22 Shield        Mega
///     D10              D53
///     D13              D52
///     D11              D51
...
/// such as the reception of a new packet, or the completion of transmission of
/// a packet. The RH_RF22 interrupt service routine reads status from and writes
/// data to the the RF22 module via the SPI interface. It is very important
/// therefore, that if you are using the RF22 library with another SPI based
/// deviced, that you disable interrupts while you transfer data to and from
/// that other device. Use cli() to disable interrupts and sei() to reenable
/// them.
///
/// \par SPI Interface
///
...
/// SPI class RHSoftwareSPI, which can be configured to work on any Arduino
/// digital IO pins. See the documentation of RHSoftwareSPI for details.
///
...
/// If you observe reliable behaviour with the default hardware SPI
/// RHHardwareSPI, but unreliable behaviour with Software SPI RHSoftwareSPI, it
/// may be due to slow CPU performance.
///
...
/// \par Memory
///
/// The RH_RF22 Driver requires non-trivial amounts of memory. The sample
/// programs all compile to about 9 to 14kbytes each on Arduino, which will fit
/// in the flash proram memory of most Arduinos. However, the RAM requirements
/// are more critical. Most sample programs above will run on Duemilanova, but
/// not on Diecimila. Even on Duemilanova, the RAM requirements are very close
/// to the available memory of 2kbytes. Therefore, you should be vary sparing
...
///
/// With an Arduino Mega, with 8 kbytes of SRAM, there is much more RAM headroom
/// for your own elaborate programs. This library is reported to work with
/// Arduino Pro Mini, but that has not been tested by me.
///
/// The RF22M modules use an inexpensive crystal to control the frequency
/// synthesizer, and therfore you can expect the transmitter and receiver
/// frequencies to be subject to the usual inaccuracies of such crystals. The
/// RF22 contains an AFC circuit to compensate for differences in transmitter
/// and receiver frequencies. It does this by altering the receiver frequency
/// during reception by up to the pull-in frequency range. This RF22 library
/// enables the AFC and by default sets the pull-in frequency range to 0.05MHz,
/// which should be sufficient to handle most situations. However, if you
/// observe unexplained packet losses or failure to operate correctly all the
/// time it may be because your modules have a wider frequency difference, and
/// you may need to set the afcPullInRange to a different value, using
/// setFrequency();
///
...
/// module.
///
/// Note: with RFM23BP, the reported maximum possible power when operating
/// on 3.3V is 27dBm.
///
/// We have made some actual power measurements against
/// programmed power for Sparkfun RFM22 wireless module under the following
/// conditions:
...
/// In general packet transmission rate will be limited by the modulation
/// scheme. Also, if your code does any slow operations like Serial printing it
/// will also limit performance. We disabled any printing in the tests below. We
/// tested with RH_RF22::GFSK_Rb125Fd125, which is probably the fastest scheme
/// available. We tested with a 13 octet message length, over a very short
/// distance of 10cm.
///
...
///
/// Transmit-and-wait-for-a-reply tests with modulation RH_RF22::GFSK_Rb125Fd125
/// and a 13 octet message (send and receive) show about 160 round trips per
/// second.
///
/// \par Compatibility with RF22 library
/// The RH_RF22 driver is based on our earlier RF22 library
/// http://www.airspayce.com/mikem/arduino/RF22 We have tried hard to be as
/// compatible as possible with the earlier RF22 library, but there are some
/// differences:
/// - Different constructor.
/// - Indexes for some modem configurations have changed (we recommend you use
/// the symbolic names, not integer indexes).
///
/// The major difference is that under RadioHead, you are
/// required to create 2 objects (ie RH_RF22 and a manager) instead of just one
/// object under RF22 (ie RHMesh, RHRouter, RHReliableDatagram or RHDatagram).
...
class RH_RF22 : public RHSPIDriver {
   public:
    ...
    /// setModemConfig() writes the register values to the appropriate RH_RF22
    /// registers to set the desired modulation type, data rate and
    /// deviation/bandwidth. Suitable values for these registers can be computed
    ...
    typedef struct {
        ...
        uint8_t reg_21;  ///< Value for register
                         ///< RH_RF22_REG_21_CLOCK_RECOVERY_OFFSET2
        uint8_t reg_22;  ///< Value for register
                         ///< RH_RF22_REG_22_CLOCK_RECOVERY_OFFSET1
        uint8_t reg_23;  ///< Value for register
                         ///< RH_RF22_REG_23_CLOCK_RECOVERY_OFFSET0
        uint8_t reg_24;  ///< Value for register
    ...
    /// types, and data rates. If you need another configuration, use the
    /// register calculator. and call setModemRegisters() with your desired
    /// settings. These are indexes into MODEM_CONFIG_TABLE. We strongly
    /// recommend you use these symbolic definitions and not their integer
    /// equivalents: its possible that new values will be introduced in later
    ...
    typedef enum {
        ...
        OOK_Rb1_2Bw75,    ///< OOK, No Manchester, Rb = 1.2kbs,  Rx Bandwidth =
                          ///< 75kHz
        OOK_Rb2_4Bw335,   ///< OOK, No Manchester, Rb = 2.4kbs,  Rx Bandwidth =
                          ///< 335kHz
        OOK_Rb4_8Bw335,   ///< OOK, No Manchester, Rb = 4.8kbs,  Rx Bandwidth =
                          ///< 335kHz
        OOK_Rb9_6Bw335,   ///< OOK, No Manchester, Rb = 9.6kbs,  Rx Bandwidth =
                          ///< 335kHz
        OOK_Rb19_2Bw335,  ///< OOK, No Manchester, Rb = 19.2kbs, Rx Bandwidth =
                          ///< 335kHz
        OOK_Rb38_4Bw335,  ///< OOK, No Manchester, Rb = 38.4kbs, Rx Bandwidth =
                          ///< 335kHz
        OOK_Rb40Bw335     ///< OOK, No Manchester, Rb = 40kbs,   Rx Bandwidth =
                          ///< 335kHz
    ...
    /// They deliberately have the same numeric values as the crc[1:0] field of
    /// Register RH_RF22_REG_30_DATA_ACCESS_CONTROL
    typedef enum {
        CRC_CCITT = 0,    ///< CCITT
    ...
    /// sufficient distinct interrupt lines, one for each instance. \param[in]
    /// slaveSelectPin the Arduino pin number of the output to use to select the
    /// RH_RF22 before accessing it. Defaults to the normal SS pin for your
    ...
    /// the RF22 NIRQ interrupt line. Defaults to pin 2, as required by sparkfun
    /// RFM22 module shields. Caution: You must specify an interrupt capable
    ...
    /// 2, 3, 18, 19, 20, 21. On Due and Teensy, any digital pin. On other
    /// Arduinos pins 2 or 3. See http://arduino.cc/en/Reference/attachInterrupt
    /// for more details. On Chipkit Uno32, pins 38, 2, 7, 8, 35. On other
    ...
    /// - Checks the connected RH_RF22 module is either a
    /// RH_RF22_DEVICE_TYPE_RX_TRX or a RH_RF22_DEVICE_TYPE_TX
    ...
    /// Reads and returns the device status register
    /// RH_RF22_REG_02_DEVICE_STATUS \return The value of the device status
    /// register
    uint8_t statusRead();

    /// Reads a value from the on-chip analog-digital converter
    /// \param[in] adcsel Selects the ADC input to measure. One of
    /// RH_RF22_ADCSEL_*. Defaults to the internal temperature sensor \param[in]
    /// adcref Specifies the refernce voltage to use. One of RH_RF22_ADCREF_*.
    ...
    /// RH_RF22_REG_18_WAKEUP_TIMER_VALUE2 \return The wakeup timer value
    uint16_t wutRead();

    /// Sets the wakeup timer period registers
    /// RH_RF22_REG_14_WAKEUP_TIMER_PERIOD1, RH_RF22_REG_15_WAKEUP_TIMER_PERIOD2
    /// and RH_RF22_R<EG_16_WAKEUP_TIMER_PERIOD3 \param[in] wtm Wakeup timer
    /// mantissa value \param[in] wtr Wakeup timer exponent R value \param[in]
    /// wtd Wakeup timer exponent D value
    void setWutPeriod(uint16_t wtm, uint8_t wtr = 0, uint8_t wtd = 0);

    /// Sets the transmitter and receiver centre frequency
    /// \param[in] centre Frequency in MHz. 240.0 to 960.0. Caution, some
    /// versions of RH_RF22 and derivatives implemented more restricted
    /// frequency ranges. \param[in] afcPullInRange Sets the AF Pull In Range in
    /// MHz. Defaults to 0.05MHz (50kHz). Range is 0.0 to 0.159375 for
    /// frequencies 240.0 to 480MHz, and 0.0 to 0.318750MHz for  frequencies
    /// 480.0 to 960MHz, \return true if the selected frquency centre + (fhch *
    /// fhs) is within range and the afcPullInRange is within range
    ...
    /// \return true if the selected frquency centre + (fhch * fhs) is within
    /// range
    bool setFHChannel(uint8_t fhch);

    /// Reads and returns the current RSSI value from register
    /// RH_RF22_REG_26_RSSI. Caution: this is in internal units (see figure 31
    /// of RFM22B/23B documentation), not in dBm. If you want to find the RSSI
    /// in dBm of the last received message, use lastRssi() instead. \return The
    /// current RSSI value
    uint8_t rssiRead();

    /// Reads and returns the current EZMAC value from register
    /// RH_RF22_REG_31_EZMAC_STATUS \return The current EZMAC value
    uint8_t ezmacStatusRead();

    /// Sets the parameters for the RH_RF22 Idle mode in register
    /// RH_RF22_REG_07_OPERATING_MODE. Idle mode is the mode the RH_RF22 will be
    /// in when not transmitting or receiving. The default idle mode is
    /// RH_RF22_XTON ie READY mode. \param[in] mode Mask of mode bits, using
    /// RH_RF22_SWRES, RH_RF22_ENLBD, RH_RF22_ENWT, RH_RF22_X32KSEL,
    /// RH_RF22_PLLON, RH_RF22_XTON.
    ...
    /// Sets the transmitter power output level in register
    /// RH_RF22_REG_6D_TX_POWER. Be a good neighbour and set the lowest power
    /// level you need. After init(), the power will be set to
    /// RH_RF22::RH_RF22_TXPOW_8DBM on RF22B or RH_RF22_RF23B_TXPOW_1DBM on an
    /// RF23B. The highest power available on RF22B is
    /// RH_RF22::RH_RF22_TXPOW_20DBM (20dBm). The highest power available on
    /// RF23B is RH_RF22::RH_RF22_RF23B_TXPOW_13DBM (13dBm). Higher powers are
    /// available on RF23BP (using RH_RF22_RF23BP_TXPOW_*), and then only with
    /// an adequate power supply. See comments above. Caution: In some countries
    /// you may only select certain higher power levels if you are also using
    /// frequency hopping. Make sure you are aware of the legal limitations and
    /// regulations in your region. \param[in] power Transmitter power level,
    /// one of RH_RF22_*TXPOW_*
    void setTxPower(uint8_t power);

    /// Sets all the registered required to configure the data modem in the
    /// RH_RF22, including the data rate, bandwidths etc. You cas use this to
    /// configure the modem with custom configuraitons if none of the canned
    /// configurations in ModemConfigChoice suit you. \param[in] config A
    /// ModemConfig structure containing values for the modem configuration
    /// registers.
    ...
    /// \return true if a complete, valid message has been received and is able
    /// to be retrieved by recv()
    bool available();
    ...
    /// call it in your main loop. \param[in] buf Location to copy the received
    /// message \param[in,out] len Pointer to available space in buf. Set to the
    /// actual number of octets copied. \return true if a valid message was
    /// copied to buf
    ...
    /// starts the transmitter. Note that a message length of 0 is NOT
    /// permitted. \param[in] data Array of data to be sent \param[in] len
    /// Number of bytes of data to send (> 0) \return true if the message length
    /// was valid and it was correctly queued for transmit
    ...
    /// RH_RF22_REG_36_SYNC_WORD3 to RH_RF22_REG_39_SYNC_WORD0 Caution:
    /// SyncWords should be set to the same value on all nodes in your network.
    /// Nodes with different SyncWords set will never receive each others
    /// messages, so different SyncWords can be used to isolate different
    ...
    /// Sets the CRC polynomial to be used to generate the CRC for both receive
    /// and transmit otherwise the default of CRC_16_IBM will be used.
    ...
    /// Configures GPIO pins for reversed GPIO connections to the antenna
    /// switch. Normally on RF22 modules, GPIO0(out) is connected to TX_ANT(in)
    /// to enable tx antenna during transmit and GPIO1(out) is connected to
    /// RX_ANT(in) to enable rx antenna during receive. The RH_RF22 driver
    /// configures the GPIO pins during init() so the antenna switch works as
    /// expected. However, some RF22 modules, such as HAB-RFM22B-BOA
    /// HAB-RFM22B-BO, also Si4432 sold by Dorji.com via Tindie.com have these
    /// GPIO pins reversed, so that GPIO0 is connected to RX_ANT. Call this
    /// function with a true argument after init() and before transmitting in
    /// order to configure the module for reversed GPIO pins. \param[in]
    /// gpioReversed Set to true if your RF22 module has reversed GPIO antenna
    /// switch connections.
    void setGpioReversed(bool gpioReversed = false);

    /// Returns the time in millis since the last preamble was received, and
    /// when the last RSSI measurement was made.
    ...
    /// takes a finite time to wake from sleep mode. \return true if sleep mode
    /// was successfully entered.
    virtual bool sleep();

   protected:
    /// This is a low level function to handle the interrupts for one instance
    /// of RH_RF22. Called automatically by isr*() Should not need to be called.
    ...
    /// \return false if the resulting message would exceed
    /// RH_RF22_MAX_MESSAGE_LEN, else true
    ...
    /// This function will be called by handleInterrupt() if an RF22 external
    /// interrupt occurs. This can only happen if external interrupts are
    /// enabled in the RF22 (which they are not by default). Subclasses may
    /// override this function to get control when an RF22 external interrupt
    /// occurs.
    virtual void handleExternalInterrupt();

    /// This function will be called by handleInterrupt() if an RF22 wakeup
    /// timer interrupt occurs. This can only happen if wakeup timer interrupts
    /// are enabled in theRF22 (which they are not by default). Subclasses may
    /// override this function to get control when an RF22 wakeup timer
    /// interrupt occurs.
    ...
    /// Sets the radio operating mode for the case when the driver is idle (ie
    /// not transmitting or receiving), allowing you to control the idle mode
    /// power requirements at the expense of slower transitions to transmit and
    /// receive modes. By default, the idle mode is RH_RF22_XTON, but eg
    /// setIdleMode(RH_RF22_PLL) will provide a much lower idle current but
    /// slower transitions. Call this function after init(). \param[in] idleMode
    /// The chip operating mode to use when the driver is idle. One of the valid
    /// definitions for RH_RF22_REG_07_OPERATING_MODE
   ...
   protected:
    ...
    /// Time in millis since the last preamble was received (and the last time
    /// the RSSI was measured)
