...
// Maximum message length (including the headers, byte count and FCS) we are
// willing to support This is pretty arbitrary
...
// Standard is if a transition occurs before RH_ASK_RAMP_TRANSITION (80) in the
// ramp, the ramp is retarded by adding RH_ASK_RAMP_INC_RETARD (11) else by
// adding RH_ASK_RAMP_INC_ADVANCE (29) If there is no transition it is adjusted
// by RH_ASK_RAMP_INC (20)
...
/// 36 alternating 1/0 bits, followed by 12 bits of start symbol (together
/// called the preamble) Followed immediately by the 4-6 bit encoded byte count,
...
/// inexpensive ASK (Amplitude Shift Keying) or OOK (On Off Keying) RF
/// transceivers.
///
/// The message format and software technology is based on our earlier
/// VirtualWire library (http://www.airspayce.com/mikem/arduino/VirtualWire),
/// with which it is compatible. See
/// http://www.airspayce.com/mikem/arduino/VirtualWire.pdf for more details.
/// VirtualWire is now obsolete and unsupported and is replaced by this library.
///
/// RH_ASK is a Driver for Arduino, Maple and others that provides features to
/// send short messages, without addressing, retransmit or acknowledgment, a bit
/// like UDP over wireless, using ASK (amplitude shift keying). Supports a
/// number of inexpensive radio transmitters and receivers. All that is required
/// is transmit data, receive data and (for transmitters, optionally) a PTT
/// transmitter enable. Can also be used over various analog connections (not
/// just a data radio), such as the audio channel of an A/V sender, or long TTL
/// lines.
///
...
///
/// http://web.engr.oregonstate.edu/~moon/research/files/cas2_mar_07_dpll.pdf
/// while not directly relevant is also interesting.
///
...
/// - 1 byte of message length byte count (4 to 30), count includes byte count
/// and FCS bytes
/// - n message bytes (uincluding 4 bytes of header), maximum n is
/// RH_ASK_MAX_MESSAGE_LEN + 4 (64)
/// - 2 bytes FCS, sent low byte-hi byte
///
/// Everything after the start symbol is encoded 4 to 6 bits, Therefore a byte
/// in the message is encoded as 2x6 bit symbols, sent hi nybble, low nybble.
/// Each symbol is sent LSBit first. The message may consist of any binary
/// digits.
///
/// The Arduino Diecimila clock rate is 16MHz => 62.5ns/cycle.
/// For an RF bit rate of 2000 bps, need 500microsec bit period.
/// The ramp requires 8 samples per bit period, so need 62.5microsec per sample
/// => interrupt tick is 62.5microsec.
///
/// The maximum packet length consists of
/// (6 + 2 + RH_ASK_MAX_MESSAGE_LEN*2) * 6 = 768 bits = 0.384 secs (at 2000
/// bps). where RH_ASK_MAX_MESSAGE_LEN is RH_ASK_MAX_PAYLOAD_LEN - 7 (= 60). The
/// code consists of an ISR interrupt handler. Most of the work is done in the
/// interrupt handler for both transmit and receive, but some is done from the
/// user level. Expensive functions like CRC computations are always done in the
/// user level.
///
...
/// - XV4Y), but untested by us. It also runs on Teensy 3.0 (courtesy of Paul
...
/// Caution: The RAM memory requirements on an ATTiny85 are *very* tight. Even
/// the bare bones ask_transmitter sketch barely fits in eh RAM available on the
/// ATTiny85. Its unlikely to work on smaller ATTinys such as the ATTiny45 etc.
/// If you have wierd behaviour, consider reducing the size of
/// RH_ASK_MAX_PAYLOAD_LEN to the minimum you can work with. Caution: the
/// default internal clock speed on an ATTiny85 is 1MHz. You MUST set the
/// internal clock speed to 8MHz. You can do this with Arduino IDE, tineycore
/// and ArduinoISP by setting the board type to "ATtiny85@8MHz', setting
/// theProgrammer to 'Arduino as ISP' and selecting Tools->Burn Bootloader. This
/// does not actually burn a bootloader into the tiny, it just changes the fuses
/// so the chip runs at 8MHz. If you run the chip at 1MHz, you will get RK_ASK
/// speeds 1/8th of the expected.
///
...
///
/// \par ESP8266
/// This module has been tested with the ESP8266 using an ESP-12 on a breakout
/// board ESP-12E SMD Adaptor Board with Power Regulator from tronixlabs
/// http://tronixlabs.com.au/wireless/esp8266/esp8266-esp-12e-smd-adaptor-board-with-power-regulator-australia/
/// compiled on Arduino 1.6.5 and the ESP8266 support 2.0 installed with Board
/// Manager. CAUTION: do not use pin 11 for IO with this chip: it will cause the
/// sketch to hang. Instead use constructor arguments to configure different
/// pins, eg: \code RH_ASK driver(2000, 2, 4, 5); \endcode Which will initialise
/// the driver at 2000 bps, recieve on GPIO2, transmit on GPIO4, PTT on GPIO5.
/// Caution: on the tronixlabs breakout board, pins 4 and 5 may be labelled
/// vice-versa.
///
/// \par Timers
/// The RH_ASK driver uses a timer-driven interrupt to generate 8 interrupts per
/// bit period. RH_ASK takes over a timer on Arduino-like platforms. By default
/// it takes over Timer 1. You can force it to use Timer 2 instead by enabling
/// the define RH_ASK_ARDUINO_USE_TIMER2 near the top of RH_ASK.cpp On Arduino
/// Zero it takes over timer TC3. On Arduino Due it takes over timer TC0. On
/// ESP8266, takes over timer0 (which conflicts with ServoTimer0).
///
...
/// of millis() etc but does permit analog outputs. This will affect the
/// accuracy of millis() and time measurement.
class RH_ASK : public RHGenericDriver {
   public:
    ...
    /// \param[in] pttPin The pin that is connected to the transmitter
    ...
    /// a message is actually received bythe transport, when it wil be returned
   ...
   protected:
    ...
    /// Set up the timer and its interrutps so the interrupt handler is called
    /// at the right frequency
    void timerSetup();

    /// Read the rxPin in a platform dependent way, taking into account whether
    ...
    /// Write the txPin in a platform dependent way, taking into account whether
    /// it is inverted or not
    ...
    /// This is the integrate and dump integral. If there are <5 0 samples in
    /// the PLL cycle the bit is declared a 0, else a 1
