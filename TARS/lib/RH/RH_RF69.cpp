...
PROGMEM static const RH_RF69::ModemConfig MODEM_CONFIG_TABLE[] = {
    ...
    //    { CONFIG_FSK,  0x0c, 0x80, 0x02, 0x8f, 0x52, 0x52, CONFIG_WHITE}, //
    //    works 10/40/80
    //    { CONFIG_FSK,  0x0c, 0x80, 0x02, 0x8f, 0x53, 0x53, CONFIG_WHITE}, //
    //    works 10/40/40
...
bool RH_RF69::init() {
    ...
    // modem config data) 0 to 60 bytes data RSSI Threshold -114dBm We dont use
    ...
    //    spiWrite(RH_RF69_REG_2E_SYNCCONFIG, RH_RF69_SYNCCONFIG_SYNCON); //
    //    auto, tolerance 0
...
// Caution: since we put our headers in what the RH_RF69 considers to be the
// payload, if encryption is enabled we have to suffer the cost of decryption
// before we can determine whether the address is acceptable. Performance issue?
void RH_RF69::readFifo() {
    ...
    ATOMIC_BLOCK_END;
    // Any junk remaining in the FIFO will be cleared next time we go to receive
    // mode.
}
...
void RH_RF69::setModeRx() {
    if (_mode != RHModeRx) {
        ...
        spiWrite(RH_RF69_REG_25_DIOMAPPING1,
                 RH_RF69_DIOMAPPING1_DIO0MAPPING_01);  // Set interrupt line 0
                                                       // PayloadReady
...
void RH_RF69::setModeTx() {
    if (_mode != RHModeTx) {
        ...
        spiWrite(RH_RF69_REG_25_DIOMAPPING1,
                 RH_RF69_DIOMAPPING1_DIO0MAPPING_00);  // Set interrupt line 0
                                                       // PacketSent
