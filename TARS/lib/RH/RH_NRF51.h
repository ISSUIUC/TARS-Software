...
/// \brief Send and receive addressed datagrams by nRF51 and nRF52 compatible
/// transceivers.
///
...
/// unaddressed, unreliable datagrams of arbitrary length to 254 octets per
...
/// is a complete SoC (system on a chip) with ARM microprocessor and 2.4 GHz
/// radio, which supports a range of channels and transmission bit rates. Chip
/// antenna is on-board.
///
/// This library provides functions for sending and receiving messages of up to
/// 254 octets on any frequency supported by the nRF51822/nRF52832, at a
/// selected data rate.
///
/// The nRF51 transceiver is configured to use Enhanced Shockburst with no
/// acknowledgement and no retransmits. TXADDRESS and RXADDRESSES:RXADDR0 (ie
/// pipe 0) are the logical address used. The on-air network address is set in
/// BASE0 and PREFIX0. SHORTS is used to automatically transition the radio
/// between Ready, Start and Disable. No interrupts are used.
///
...
/// It is NOT compatible with the one used by RH_NRF24 and the nRF24L01 product
/// specification, mainly because the nRF24 only supports 6 bits of message
/// length.
///
/// - 1 octets PREAMBLE
...
///
/// The sample programs are designed to be built using Arduino 1.6.4 or later
/// using the procedures outlined in
/// http://redbearlab.com/getting-started-nrf51822/ or with Sparkfun nRF52832
/// breakout board, with Arduino 1.6.13 and Sparkfun nRF52 boards manager 0.2.3
/// using the procedures outlined in
/// https://learn.sparkfun.com/tutorials/nrf52832-breakout-board-hookup-guide
///
...
///
/// The RF powers claimed by the chip manufacturer have not been independently
/// verified here.
///
/// \par Memory
///
/// The compiled client and server sketches are about 42k bytes on Arduino.
/// The reliable client and server sketches compile to about 43k bytes on
/// Arduino. Unfortunately the Arduino build environmnet does not drop unused
/// clsses and code, so the resulting programs include all the unused classes ad
/// code. This needs to be revisited. RAM requirements are minimal.
///
class RH_NRF51 : public RHGenericDriver {
   public:
    ...
    /// - Set the logical channel to 0 for transmit and receive (only pipe 0 is
    /// used)
    /// - Configure the CRC (2 octets, algorithm x^16+x^12^x^5+1 with initial
    /// value 0xffff)
    ...
    /// nodes to isolate them from each other. Internally, this sets the nRF51
    /// BASE0 and PREFIX0 to be the given network address. The first octet of
    /// the address is used for PREFIX0 and the rest is used for BASE0. BALEN is
    /// set to the approprtae base length.
    /// The default network address is 0xE7E7E7E7E7.
    /// \param[in] address The new network address. Must match the network
    /// address of any receiving node(s). \param[in] len Number of bytes of
    /// address to set (3 to 5). \return true on success, false if len is not in
    /// the range 3-5 inclusive.
    bool setNetworkAddress(uint8_t* address, uint8_t len);

    /// Sets the data rate and transmitter power to use.
    /// \param [in] data_rate The data rate to use for all packets transmitted
    /// and received. One of RH_NRF51::DataRate. \param [in] power Transmitter
    /// power. One of RH_NRF51::TransmitPower. \return true on success
    ...
    /// Caution: when encryption is enabled, the maximum message length is
    /// reduced to 23 octets. \param [in] data Data bytes to send. \param [in]
    /// len Number of data bytes to send \return true on success (which does not
    /// necessarily mean the receiver got the message, only that the message was
    /// successfully transmitted). False if the message is too long or was
    /// otherwise not transmitted.
    ...
    /// platform For debugging purposes only. Caution: there are 1024 of them
    /// (many reserved and set to 0). \return true on success
    bool printRegisters();
    ...
    /// Once a message with CRC correct is received, the receiver will be
    /// returned to Idle mode. If there is a valid message available, copy it to
    /// buf and return true else return false. If a message is copied, *len is
    /// set to the length (Caution, 0 length messages are permitted). You should
    /// be sure to call this function frequently enough to not miss any messages
    ...
    /// to encrypt and decrypt all messages using the on-chip AES CCM mode
    /// encryption engine. The default is disabled. In the AES configuration,
    /// the message counter and IV is always set to 0, which means the same
    /// keystream is used for every message with a given key. Caution: when
    /// encryption is enabled, the maximum message length is reduced to 23
    /// octets. \param[in] key The key to use. Must be 16 bytes long. The same
    /// key must be installed in other instances of RH_RF51, otherwise
    /// communications will not work correctly. If key is NULL, encryption is
    ...
    /// Blocks while the temperature is measured, which takes about 30
    /// microseconds.
   ...
   private:
    ...
    /// True if an AES key has been specified and that we are therfore
    /// encrypting and decrypting messages on the fly
    bool _encrypting;
