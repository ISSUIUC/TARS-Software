...
// Each interrupt can be handled by a different instance of RH_RF24, allowing
// you to have 2 or more RF24s per Arduino
...
bool RH_RF24::init() {
    ...
    uint8_t buf[8];
    if (!command(RH_RF24_CMD_PART_INFO, 0, 0, buf, sizeof(buf))) return false;  // SPI error? Not connected?
    _deviceType = (buf[1] << 8) | buf[2];
    // Check PART to be either 0x4460, 0x4461, 0x4463, 0x4464
    if (_deviceType != 0x4460 && _deviceType != 0x4461 && _deviceType != 0x4463 && _deviceType != 0x4464)
        return false;  // Unknown radio type, or not connected

    // Here we use a configuration generated by the Silicon Labs Wireless
    // Development Suite #included above We override a few things later that we
    // ned to be sure of.
    ...
    // Ensure we get the interrupts we need, irrespective of whats in the
    // radio_config
    ...
    // PKT_TX_THRESHOLD and PKT_RX_THRESHOLD should be set to about 0x30 in
    // radio_config
    ...
    // Hmmm, having no CRC on field 1 and CRC on field 2 causes CRC errors when
    // resetting after an odd number of packets! Anyway its prob a good thing at
    // the cost of some airtime. Hmmm, enabling WHITEN stops it working!
    ...
    // Clear all other fields so they are never used, irrespective of the
    // radio_config
...
void RH_RF24::handleInterrupt() {
    ...
    if (status[0] & RH_RF24_INT_STATUS_MODEM_INT_STATUS) {
        ...
        if (status[4] & RH_RF24_INT_STATUS_INVALID_SYNC) {
            // After INVALID_SYNC, sometimes the radio gets into a silly state
            // and subsequently reports it for every packet Need to reset the
            // radio and clear the RX FIFO, cause sometimes theres junk there
            // too
            _mode = RHModeIdle;
            clearRxFifo();
            clearBuffer();
    ...
    if (status[0] & RH_RF24_INT_STATUS_PH_INT_STATUS) {
        ...
        if (status[2] & RH_RF24_INT_STATUS_PACKET_SENT) {
            ...
            // RH_RF24 configured to transition automatically to Idle after
            // packet sent
            _mode = RHModeIdle;
            clearBuffer();
        }
...
bool RH_RF24::send(const uint8_t* data, uint8_t len) {
    ...
    // First the length in fixed length field 1. This wont appear in the
    // receiver fifo since we have turned off IN_FIFO in PKT_LEN
...
void RH_RF24::setModemRegisters(const ModemConfig* config) {
    Serial.println(
        "Programming Error: setModemRegisters is obsolete. Generate custom "
        "radio config file with WDS instead");
}
...
bool RH_RF24::setFrequency(float centre, float afcPullInRange) {
    ...
    if (!set_properties(RH_RF24_PROPERTY_MODEM_CLKGEN_BAND, modem_clkgen, sizeof(modem_clkgen))) return false;
...
void RH_RF24::setModeIdle() {
    if (_mode != RHModeIdle) {
        // Set the antenna switch pins using the GPIO, assuming we have an RFM
        // module with antenna switch
...
bool RH_RF24::sleep() {
    if (_mode != RHModeSleep) {
        // This will change to SLEEP or STANDBY, depending on the value of
        // GLOBAL_CLK_CFG:CLK_32K_SEL. which default to 0, eg STANDBY
...
void RH_RF24::setModeRx() {
    if (_mode != RHModeRx) {
        // CAUTION: we cant clear the rx buffers here, else we set up a race
        // condition with the _rxBufValid test in available()

        // Tell the receiver the max data length we will accept (a TX may have
        // changed it)
...
void RH_RF24::setModeTx() {
    if (_mode != RHModeTx) {
        ...
        uint8_t tx_params[] = {
            0x00, (uint8_t)((_idleMode << 4) | RH_RF24_CONDITION_RETRANSMIT_NO | RH_RF24_CONDITION_START_IMMEDIATE)};
...
void RH_RF24::setTxPower(uint8_t power) {
    ...
    // you may get lower power from RFM modules, depending on Vcc voltage,
    // antenna etc
...
bool RH_RF24::configure(const uint8_t* commands) {
    ...
    while (memcpy_P(&next_cmd_len, commands, 1), next_cmd_len > 0) {
        uint8_t buf[20];  // As least big as the biggest permitted
                          // command/property list of 15
...
void RH_RF24::power_on_reset() {
    ...
    digitalWrite(_sdnPin,
                 HIGH);  // So we dont get a glitch after setting pinMode OUTPUT
