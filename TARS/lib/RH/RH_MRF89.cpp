...
bool RH_MRF89::init() {
    ...
    // Sigh on teensy at 1MHz, need special delay after writes, see
    ...
    // No way to check the device type but lets trivially check there is
    // something there by trying to change a register:
    spiWriteRegister(RH_MRF89_REG_02_FDEVREG, 0xaa);
    if (spiReadRegister(RH_MRF89_REG_02_FDEVREG) != 0xaa) return false;
    spiWriteRegister(RH_MRF89_REG_02_FDEVREG,
                     0x3);  // Back to the default for FDEV
    ...
    spiWriteRegister(RH_MRF89_REG_1A_TXCONREG,
    ...
    if (!setFrequency(915.4)) return false;
...
// MRF89XA is unusual in that it has 2 interrupt lines, and not a single,
// combined one. Only one of the several interrupt lines (IRQ1) from the RFM95
// needs to be connnected to the processor. We use this to get CRCOK and TXDONE
// interrupts
...
uint8_t RH_MRF89::spiWriteRegister(uint8_t reg, uint8_t val) {
    ...
    setSlaveSelectPin(_csconPin);
    digitalWrite(_csdatPin, HIGH);
    // Hmmm, on teensy 3.1, needed some special behaviour in
    // RHNRFSPIDriver::spiWrite because otherwise, CSCON returns high before the
    // final clock goes low, which prevents the MRF89XA spi write succeeding.
    // Clock must be low when CSCON goes high.
    return spiWrite(((reg & 0x1f) << 1), val);
}

uint8_t RH_MRF89::spiWriteData(uint8_t data) {
    spiWriteRegister(RH_MRF89_REG_1F_FCRCREG,
                     RH_MRF89_ACFCRC);  // Write to FIFO
...
uint8_t RH_MRF89::spiReadData() {
    spiWriteRegister(RH_MRF89_REG_1F_FCRCREG,
                     RH_MRF89_ACFCRC | RH_MRF89_FRWAXS);  // Read from FIFO
    setSlaveSelectPin(_csdatPin);
    digitalWrite(_csconPin, HIGH);
    return spiCommand(0);
}
...
bool RH_MRF89::verifyPLLLock() {
    ...
    spiWriteRegister(RH_MRF89_REG_0E_FTPRIREG,
                     ftpriVal | RH_MRF89_LSTSPLL);  // Clear PLL lock bit
...
bool RH_MRF89::setModemConfig(ModemConfigChoice index) {
    ...
    // The sample configs in MRF89XA.h all use TXIPOLFV = 0xf0 => 375kHz, which
    // is too wide for most modulations
