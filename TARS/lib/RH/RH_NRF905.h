...
// This is the maximum (and only) number of bytes that can be carried by the
// nRF905. We use some for headers, leaving fewer for RadioHead messages
...
/// nRF905 and compatible transceivers.
///
...
///
/// The nRF905 transceiver is configured to use Enhanced Shockburst with 16 Bit
/// CRC, and 32 octet packets.
///
/// Naturally, for any 2 radios to communicate that must be configured to use
/// the same frequency and with identical network addresses.
///
/// The nRF905 from Nordic Semiconductor
...
/// is a low-cost 433/868/915 MHz ISM transceiver module. It supports a number
/// of channel frequencies at 100kHz deviation and 50kHz bandwidth with
/// Manchester encoding.
///
...
/// 27 octets on any frequency supported by the nRF905.
///
/// Several nRF905 modules can be connected to an Arduino, permitting the
...
/// \par Packet Format
///
/// All messages sent and received by this class conform to this fixed length
/// packet format
///
...
///   - 0 to 27 octets of user message, trailing octets after the user message
///   length are ignored
/// - 2 octets CRC
///
/// All messages sent and received by this driver are 32 octets. The user
/// message length is embedded in the message.
///
/// \par Connecting nRF905
///
/// The nRF905 is a 3.3V part is is *NOT* 5V tolerant. So you MUST use a 3.3V
/// CPU such as Teensy, Arduino Due etc or else provide for level shifters
/// between the CPU and the nRF905. Failure to consider this will probably break
/// your nRF905.
///
/// The electrical connection between the nRF905 and the CPU require 3.3V, the 3
/// x SPI pins (SCK, SDI, SDO), a Chip Enable pin, a Transmit Enable pin and a
/// Slave Select pin.
///
/// The examples below assume the commonly found cheap Chinese nRF905 modules.
/// The RH_RF905 driver assumes the the nRF905 has a 16MHz crystal.
///
/// Connect the nRF905 to Teensy (or Arduino with suitable level shifters) like
/// this \code
///                 CPU          nRF905 module
///                 3V3----------VCC   (3.3V)
...
/// \endcode
///
/// Caution: Arduino Due is a 3.3V part and is not 5V tolerant (so too is the
/// nRF905 module so they can be connected directly together. Unlike other
/// Arduinos the Due has it default SPI connections on a dedicated 6 pin SPI
/// header in the center of the board, which is physically compatible with Uno,
/// Leonardo and Mega2560. A little dot marks pin 1 on the header. You must
/// connect to these and *not* to the usual Arduino SPI pins Digital 11, 12
/// and 13. See
...
/// in the NRF905() constructor if you wish to connect the slave select CSN to
/// other than the normal one for your CPU.
///
/// It is possible to have 2 radios conected to one CPU, provided each radio has
/// its own CSN, TX_EN and CE line (SCK, MOSI and MISO are common to both
/// radios)
///
/// \par Transmitter Power
///
/// You can control the transmitter power to be one of 4 power levels: -10, -2,
/// 6 or 10dBm, using the setRF() function, eg: \code
/// nrf905.setRF(RH_NRF905::TransmitPower10dBm);
/// \endcode
///
/// We have made some actual power measurements against
/// programmed power for an nRF905 module from www.rfinchina.com under the
/// following conditions:
/// - Teensy 3.1
/// - nRF905 module (with SMA antenna connector) wired to Teensy as described
/// above, channel 108.
/// - 20cm SMA-SMA cable
...
///
/// Several example programs are provided. They work out of the box with
/// Teensy 3.1 and Arduino Due connected as show above.
///
...
/// sketches are about 16000 bytes on Teensy.
///
class RH_NRF905 : public RHNRFSPIDriver {
   public:
    /// \brief Convenient values for setting transmitter power in setRF()
    /// These are designed to agree with the values for
    /// RH_NRF905_CONFIG_1_PA_PWR after left shifting by 2 To be passed to
    /// setRF();
    typedef enum {
    ...
    /// transmit/receive \param[in] txEnablePin the Arduino pin cponnected to
    /// the txEn pin on the radio that enable transmit mode \param[in]
    /// slaveSelectPin the Arduino pin number of the output to use to select the
    /// NRF905 before accessing it. Defaults to the normal SS pin for your
    ...
    /// Writes a single byte to the NRF905, and at the ame time reads the
    /// current STATUS register \param[in] reg Register number, one of
    /// NRF905_REG_* \param[in] val The value to write \return the current
    /// STATUS (read while the command is sent)
    uint8_t spiWriteRegister(uint8_t reg, uint8_t val);

    /// Reads a number of consecutive registers from the NRF905 using burst read
    /// mode \param[in] reg Register number of the first register, one of
    /// NRF905_REG_* \param[in] dest Array to write the register values to. Must
    /// be at least len bytes \param[in] len Number of bytes to read \return the
    /// current STATUS (read while the command is sent)
    ...
    /// NRF905_REG_* \param[in] src Array of new register values to write. Must
    /// be at least len bytes \param[in] len Number of bytes to write \return
    ...
    /// \param[in] hiFrequency false for low frequency band (422.4MHz and up),
    /// true for high frequency band (845MHz and up) \return true on success
    ...
    /// nodes to isolate them from each other. The default network address is
    /// 0xE7E7E7E7 \param[in] address The new network address. Must match the
    /// network address of any receiving node(s). \param[in] len Number of bytes
    /// of address to set (1 to 4). \return true on success, false if len is not
    /// in the range 1-4 inclusive.
    ...
    /// \param [in] len Number of data bytes to set in the TX buffer. The actual
    /// size of the transmitted data payload is set by setPayloadSize. Maximum
    /// message length actually transmitted is RH_NRF905_MAX_MESSAGE_LEN = 27.
    ...
    /// Returns false if the requested message length exceeds
